...
@Service
public class YihaodianInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(YihaodianInsertCwbDetailTimmer.class);
	...
	public void selectTempAndInsertToCwbDetail(int yhd_key) {
		...
		Yihaodian yihaodian = yihaodianService.getYihaodian(yhd_key);
		...
		while (true) {
			...
			ImportSubList(yihaodian, subList);
			k++;
		}
	...
	@Transactional
	public void ImportSubList(Yihaodian yihaodian, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(yihaodian, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(Yihaodian yihaodian, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = yihaodian.getWarehouseid();
