...
@Service
public class HxgdmsInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(HxgdmsInsertCwbDetailTimmer.class);

	@Autowired
	HxgdmsService smileService;
	@Autowired
	...
	public void selectTempAndInsertToCwbDetail() {
		int isOpenFlag = jointService.getStateForJoint(B2cEnum.Hxgdms.getKey());
		if (isOpenFlag == 0) {
			logger.info("未开启[好享购DMS]对接！");
			return;
		}
		Hxgdms hxgdms = smileService.getHxgDms(B2cEnum.Hxgdms.getKey());
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(hxgdms.getCustomerids());
		...
		while (true) {
			...
			ImportSubList(hxgdms, subList);
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Hxgdms smile, List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Hxgdms hxgdms, CwbOrderDTO cwbOrder) {
		CwbOrder order = cwbDAO.getCwbByCwb(cwbOrder.getCwb());
		if (order != null) { // 要合单子
			logger.warn("查询临时表-检测到0好享购DMS0有重复数据,已过滤!订单号：{},运单号:{}", cwbOrder.getCwb(), cwbOrder.getShipcwb());
		} else {
			// insert
			User user = new User();
			user.setUserid(1);
