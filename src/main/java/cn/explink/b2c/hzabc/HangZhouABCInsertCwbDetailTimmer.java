...
@Service
public class HangZhouABCInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(HangZhouABCInsertCwbDetailTimmer.class);
	...
	public long selectTempAndInsertToCwbDetail(int key) {
		...
		HangZhouABC hzabc = hangZhouABCService.getHangZhougABC(key);
		int isOpenFlag = jointService.getStateForJoint(key);
		if (isOpenFlag == 0) {
			logger.info("未开启0杭州ABC0对接！yhd_key={},当前获取订单插入临时表-----", key);
			return -1;
		}
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(hzabc.getCustomerids());
	...
	@Transactional
	public void ImportSubList(HangZhouABC hzabc, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(hzabc, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(HangZhouABC hzabc, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = hzabc.getExportbranchid();
