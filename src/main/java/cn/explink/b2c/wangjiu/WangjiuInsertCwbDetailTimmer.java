...
@Service
public class WangjiuInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(WangjiuInsertCwbDetailTimmer.class);

	@Autowired
	WangjiuService smileService;
	@Autowired
	...
	public void selectTempAndInsertToCwbDetail() {
		int isOpenFlag = jointService.getStateForJoint(B2cEnum.Wangjiu.getKey());
		if (isOpenFlag == 0) {
			logger.info("未开启[网酒网]对接！");
			return;
		}
		Wangjiu wj = smileService.getWangjiu(B2cEnum.Wangjiu.getKey());
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(wj.getCustomerid());
		...
		while (true) {
			...
			ImportSubList(wj, subList);
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Wangjiu smile, List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Wangjiu hxgdms, CwbOrderDTO cwbOrder) {
		CwbOrder order = cwbDAO.getCwbByCwb(cwbOrder.getCwb());
		if (order != null) { // 要合单子
			logger.warn("查询临时表-检测到0网酒网0有重复数据,已过滤!订单号：{},运单号:{}", cwbOrder.getCwb(), cwbOrder.getShipcwb());
		} else {
			// insert
			User user = new User();
			user.setUserid(1);
