...
@Service
public class BenlaiInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(BenlaiInsertCwbDetailTimmer.class);
	...
	public void selectTempAndInsertToCwbDetail(int yhd_key) {
		...
		BenLaiShengHuo benlai = benLaiShengHuoService.getBenlaiShenghuo(yhd_key);
		int isOpenFlag = jointService.getStateForJoint(yhd_key);
		if (isOpenFlag == 0) {
			logger.info("未开启[本来]对接！yhd_key={},当前获取订单插入临时表-----", yhd_key);
			return;
		}
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(benlai.getCustomerid());
	...
	@Transactional
	public void ImportSubList(BenLaiShengHuo benlai, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(benlai, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(BenLaiShengHuo benlai, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = benlai.getWarehouseid();
