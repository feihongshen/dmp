...
@Service
public class JiuxianInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(JiuxianInsertCwbDetailTimmer.class);
	...
	public void selectTempAndInsertToCwbDetail(int yhd_key) {
		...
		JiuxianWang jiuxian = jiuxianService.getJiuxianShenghuo(yhd_key);
		int isOpenFlag = jointService.getStateForJoint(yhd_key);
		if (isOpenFlag == 0) {
			logger.info("未开启[本来]对接！yhd_key={},当前获取订单插入临时表-----", yhd_key);
			return;
		}
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(jiuxian.getCustomerid());
	...
	@Transactional
	public void ImportSubList(JiuxianWang jiuxian, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(jiuxian, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(JiuxianWang jiuxian, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = jiuxian.getWarehouseid();
