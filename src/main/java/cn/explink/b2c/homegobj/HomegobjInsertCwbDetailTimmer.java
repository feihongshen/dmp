...
@Service
public class HomegobjInsertCwbDetailTimmer {
	...
	public void selectTempAndInsertToCwbDetail() {
		...
		int efastKey = B2cEnum.HomegoBJ.getKey();
		int isOpenFlag = jointService.getStateForJoint(efastKey);
		if (isOpenFlag == 0) {
			logger.info("未开启[家有购物北京]对接！efastKey={}", efastKey);
			return;
		}
	...
	private void dealWithEfastMethod(int efastKey) {
		...
		Homegobj hg = homegobjService.getHomegobj(efastKey);
	...
	@Transactional
	public void ImportSubList(Homegobj hg, List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Homegobj hg, CwbOrderDTO cwbOrder) {
