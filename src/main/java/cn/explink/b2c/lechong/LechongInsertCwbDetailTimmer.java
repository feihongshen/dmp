...
@Service
public class LechongInsertCwbDetailTimmer {
	...
	public void selectTempAndInsertToCwbDetail() {
		int efastKey = B2cEnum.LeChong.getKey();
		int isOpenFlag = jointService.getStateForJoint(efastKey);
		if (isOpenFlag == 0) {
			logger.info("未开启[乐宠]对接！efastKey={}", efastKey);
			return;
		}
	...
	private void dealWithEfastMethod(int efastKey) {
		Lechong le = lechongService.getLechong(efastKey);
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(le.getCustomerids());
		...
		while (true) {
			...
			ImportSubList(le, subList);
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Lechong le, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(le, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(Lechong le, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = le.getWarehouseid();
