...
@Service
public class SfxhmInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(SfxhmInsertCwbDetailTimmer.class);
	@Autowired
	...
	public void selectTempAndInsertToCwbDetail() {
		...
		int efastKey = B2cEnum.SFexpressXHM.getKey();
		int isOpenFlag = jointService.getStateForJoint(efastKey);
		if (isOpenFlag == 0) {
			logger.info("未开启[顺丰-小红帽]对接！efastKey={}", efastKey);
			return;
		}
	...
	private void dealWithEfastMethod(int efastKey) {
		...
		Sfxhm hg = sfxhmService.getSfxhm(efastKey);
	...
	@Transactional
	public void ImportSubList(Sfxhm hg, List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Sfxhm hg, CwbOrderDTO cwbOrder) {
