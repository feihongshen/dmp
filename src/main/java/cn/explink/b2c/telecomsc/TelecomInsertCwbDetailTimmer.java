...
@Service
public class TelecomInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(TelecomInsertCwbDetailTimmer.class);
	...
	/**
	 * 电信商城定时器，查询临时表，插入数据到detail表中。
	 */
	public long selectTempAndInsertToCwbDetail(int b2cKey) {
		...
		if (isOpenFlag == 0) {
			logger.info("未开启{电信商城}对接,当前获取订单插入临时表-----", b2cKey);
			return -1;
		}
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(telecom.getCustomerid());
	...
	@Transactional
	public void ImportSubList(Telecomshop telecom, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(telecom, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(Telecomshop telecom, CwbOrderDTO cwbOrder) {
		...
		if (order != null) {
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = telecom.getWarehouseid();
