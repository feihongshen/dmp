...
@Service
public class SuNingInsertCwbDetailTimmer {
	private Logger logger =LoggerFactory.getLogger(SuNingInsertCwbDetailTimmer.class);
	...
	/**
	 * 【苏宁易购】定时器，查询临时表，插入数据到detail表中。 
	 */
	private void execute(int key){
		
		SuNing suning=this.suNingService.getSuNing(key);
		List<CwbOrderDTO> cwbOrderList=dataImportDAO_B2c.getCwbOrderTempByKeys(suning.getCustomerid());
		...
		while(true){
			...
			ImportSubList(suning, subList); 
			k++;
		}
	}
	
	@Transactional
	public void ImportSubList(SuNing suning,List<CwbOrderDTO> cwbOrderList) {
		
		for(CwbOrderDTO cwbOrder:cwbOrderList){
			try {
				ImportSignOrder(suning, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(SuNing suning, CwbOrderDTO cwbOrder) {
		CwbOrder order=cwbDAO.getCwbByCwb(cwbOrder.getCwb());
		if (order!=null) {  // 要合单子
			logger.warn("查询临时表-检测到【苏宁易购】有重复数据,已过滤!订单号：{},运单号:{}",cwbOrder.getCwb(),cwbOrder.getShipcwb());
		}else{
			User user=new User();
			user.setUserid(1);
			long warehouse_id=suning.getWarehouseid();
