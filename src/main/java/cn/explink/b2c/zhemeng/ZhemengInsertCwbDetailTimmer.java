...
@Service
public class ZhemengInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(ZhemengInsertCwbDetailTimmer.class);
	...
	public void selectTempAndInsertToCwbDetailTask() {
		...
		selectTempAndInsertToCwbDetail(B2cEnum.ZheMeng.getKey());
	...
	public void selectTempAndInsertToCwbDetail(int key) {
		...
		try {
			Zhemeng dp = jiaxingService.getZhenMeng(key);
			int isOpenFlag = jointService.getStateForJoint(key);
			if (isOpenFlag == 0) {
				logger.info("未开启对接-----" + key);
				return;
			}

			List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(dp.getCustomerid());
			...
			while (true) {
				...
				ImportSubList(dp.getCustomerid(), dp.getWarehouseid(), subList);
				k++;
			}

		} catch (Exception e) {
			logger.error("0嘉兴速递0定时器临时表插入或修改方法执行异常!异常原因:", e);
		}
	}
	...
	private void ImportSignOrder(String customerid, long warehouseId, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			logger.warn("[嘉兴速递]查询临时表-检测到有重复数据,已过滤!订单号：{},运单号:{}", cwbOrder.getCwb(), cwbOrder.getShipcwb());
		} else {
			User user = new User();
			user.setUserid(1);
			...
			logger.info("[嘉兴速递]定时器临时表插入detail表成功!cwb={},shipcwb={}", cwbOrder.getCwb(), cwbOrder.getShipcwb());
