...
@Service
public class EfastInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(EfastInsertCwbDetailTimmer.class);
	@Autowired
	...
	private void dealWithEfastMethod(int efastKey) {
		...
		Efast efast = efastService.getEfast(efastKey);
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(efast.getCustomerid());
		...
		while (true) {
			...
			ImportSubList(efast, subList);
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Efast yixun, List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Efast efast, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = efast.getWarehouseid();
