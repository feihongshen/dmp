...
@Service
public class SmileInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(SmileInsertCwbDetailTimmer.class);
	...
	public void selectTempAndInsertToCwbDetail() {
		int isOpenFlag = jointService.getStateForJoint(B2cEnum.Smile.getKey());
		if (isOpenFlag == 0) {
			logger.info("未开启[广州思迈速递]对接！");
			return;
		}
		Smile smile = smileService.getSmile(B2cEnum.Smile.getKey());
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(smile.getCustomerids());
		...
		while (true) {
			...
			ImportSubList(smile, subList);
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Smile smile, List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Smile smile, CwbOrderDTO cwbOrder) {
		CwbOrder order = cwbDAO.getCwbByCwb(cwbOrder.getCwb());
		if (order != null) { // 要合单子
			logger.warn("查询临时表-检测到0广州思迈速递0有重复数据,已过滤!订单号：{},运单号:{}", cwbOrder.getCwb(), cwbOrder.getShipcwb());
		} else {
			// insert
			User user = new User();
			user.setUserid(1);
			long warehouse_id = smile.getWarehouseid();
