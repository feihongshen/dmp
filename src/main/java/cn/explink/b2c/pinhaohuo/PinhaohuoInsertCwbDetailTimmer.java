...
@Service
public class PinhaohuoInsertCwbDetailTimmer {
	private Logger logger =LoggerFactory.getLogger(PinhaohuoInsertCwbDetailTimmer.class);
	

	@Autowired
	PinhaohuoService smileService;
	@Autowired
	...
	public void selectTempAndInsertToCwbDetail(){
		int isOpenFlag=jointService.getStateForJoint(B2cEnum.PinHaoHuo.getKey());
		if (isOpenFlag == 0) {
			logger.info("未开启[拼好货]对接！");
			return;
		}
		Pinhaohuo p=smileService.getPinhaohuo(B2cEnum.PinHaoHuo.getKey());
		String customerids=p.getCustomerArrs();
		...
		while(true){
			...
			ImportSubList(p, subList); 
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Pinhaohuo smile,List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Pinhaohuo  hxgdms, CwbOrderDTO cwbOrder) {
		CwbOrder order=cwbDAO.getCwbByCwb(cwbOrder.getCwb());
		if (order!=null) {  // 要合单子
			logger.warn("查询临时表-检测到拼好货有重复数据,已过滤!订单号：{},运单号:{}",cwbOrder.getCwb(),cwbOrder.getShipcwb());
		}else{
			//insert
			User user=new User();
			user.setUserid(1);
