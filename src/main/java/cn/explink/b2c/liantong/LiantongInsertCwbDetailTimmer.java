...
@Service
public class LiantongInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(LiantongInsertCwbDetailTimmer.class);
	@Autowired
	...
	public void selectTempAndInsertToCwbDetail() {
		...
		if (isOpenFlag == 0) {
			logger.info("未开启[联通]对接！");
			return;
		}
		Liantong liantong = liantongService.getLianTong(B2cEnum.Liantong.getKey());
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(liantong.getCustomerid());
		...
		while (true) {
			...
			ImportSubList(liantong, subList);
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Liantong liantong, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(liantong, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(Liantong liantong, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = liantong.getWarehouseid();
