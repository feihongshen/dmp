...
@Service
public class EpaiInsertCwbDetailTimmer {
	private Logger logger = LoggerFactory.getLogger(EpaiInsertCwbDetailTimmer.class);
	...
	public void selectTempAndInsertToCwbDetail() {
		...
		if (epailist == null || epailist.size() == 0) {
			logger.warn("临时表更新主表-当前没有配置系统对接设置！");
			return;
		}
		for (EpaiApi epai : epailist) {
			if (epai.getIsopenflag() == 1) {
				selectTempAndInsertToCwbDetail(epai);
			}
	...
	public void selectTempAndInsertToCwbDetail(EpaiApi epai) {

		int isOpenFlag = epai.getState();
		if (isOpenFlag == 0) {
			logger.info("未开启0易派系统0对接！userCode={},当前获取订单插入临时表-----", epai.getUserCode());
			return;
		}
		List<CwbOrderDTO> cwbOrderList = dataImportDAO_B2c.getCwbOrderTempByKeys(String.valueOf(epai.getCustomerid()));
	...
	@Transactional
	public void ImportSubList(EpaiApi epai, List<CwbOrderDTO> cwbOrderList) {

		for (CwbOrderDTO cwbOrder : cwbOrderList) {
			try {
				ImportSignOrder(epai, cwbOrder);
			} catch (Exception e) {
	...
	private void ImportSignOrder(EpaiApi epai, CwbOrderDTO cwbOrder) {
		...
		if (order != null) { // 要合单子
			...
			User user = new User();
			user.setUserid(1);
			long warehouse_id = epai.getWarehouseid();
