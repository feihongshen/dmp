...
@Service
public class HuanqiugouInsertCwbDetailTimmer {
	private Logger logger =LoggerFactory.getLogger(HuanqiugouInsertCwbDetailTimmer.class);
	

	@Autowired
	HuanqiugouService smileService;
	@Autowired
	...
	public void selectTempAndInsertToCwbDetail(){
		int isOpenFlag=jointService.getStateForJoint(B2cEnum.HuanQiuGou.getKey());
		if (isOpenFlag == 0) {
			logger.info("未开启[环球购物]对接！");
			return;
		}
		Huanqiugou hqg=smileService.getHuanqiugou(B2cEnum.HuanQiuGou.getKey());
		List<CwbOrderDTO> cwbOrderList=dataImportDAO_B2c.getCwbOrderTempByKeys(hqg.getCustomerids());
		...
		while(true){
			...
			ImportSubList(hqg, subList); 
			k++;
		}
	}

	@Transactional
	public void ImportSubList(Huanqiugou smile,List<CwbOrderDTO> cwbOrderList) {
	...
	private void ImportSignOrder(Huanqiugou  hxgdms, CwbOrderDTO cwbOrder) {
		CwbOrder order=cwbDAO.getCwbByCwb(cwbOrder.getCwb());
		if (order!=null) {  // 要合单子
			logger.warn("查询临时表-检测到0环球购物0有重复数据,已过滤!订单号：{},运单号:{}",cwbOrder.getCwb(),cwbOrder.getShipcwb());
		}else{
			//insert
			User user=new User();
			user.setUserid(1);
			long warehouse_id=hxgdms.getWarehouseid();
