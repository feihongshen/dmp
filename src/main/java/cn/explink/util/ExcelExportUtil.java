...
import cn.explink.util.poi.excel.entity.ComparatorExcelField;
import cn.explink.util.poi.excel.entity.ExcelExportEntity;
import cn.explink.util.poi.excel.entity.ExcelTitle;
import cn.explink.util.poi.excel.entity.TemplateExportParams;
...
public final class ExcelExportUtil {
	 ...
	 * @throws Exception
	 */
	public static int createCells(Drawing patriarch, int index, Object t, List<ExcelExportEntity> excelParams, Sheet sheet, HSSFWorkbook workbook, Map<String, HSSFCellStyle> styles) throws Exception {
		...
		for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getList() != null) {
				...
				if (entity.getType() == 1) {
					ExcelExportUtil.createStringCell(row, cellNum++, value == null ? "" : value.toString(), (index % 2) == 0 ? ExcelExportUtil.getStyles(styles, false, entity.isWrap())
							: ExcelExportUtil.getStyles(styles, true, entity.isWrap()), entity);
				} else {
	...
	private static Object getCellValue(ExcelExportEntity entity, Object obj) throws Exception {
		Object value = entity.getGetMethods() != null ? ExcelExportUtil.getFieldBySomeMethod(entity.getGetMethods(), obj) : entity.getGetMethod().invoke(obj, new Object[] {});
	...
	private static void createListCells(Drawing patriarch, int index, int cellNum, Object obj, List<ExcelExportEntity> excelParams, Sheet sheet, HSSFWorkbook workbook,
		...
		for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getType() == 1) {
				ExcelExportUtil.createStringCell(row, cellNum++, value == null ? "" : value.toString(), (row.getRowNum() % 2) == 0 ? ExcelExportUtil.getStyles(styles, false, entity.isWrap())
						: ExcelExportUtil.getStyles(styles, true, entity.isWrap()), entity);
			} else {
	...
	public static void setCellWith(List<ExcelExportEntity> excelParams, Sheet sheet) {
		int index = 0;
	 ...
	 * @param index
	 */
	public static void createTitleRow(ExcelTitle title, Sheet sheet, HSSFWorkbook workbook, int index, List<ExcelExportEntity> excelParams) {
		Row row = sheet.createRow(index);
		...
		for (int i = 0, exportFieldTitleSize = excelParams.size(); i < exportFieldTitleSize; i++) {
			...
			ExcelExportUtil.createStringCell(row, cellIndex, entity.getName(), titleStyle, entity);
			if (entity.getList() != null) {
				List<ExcelExportEntity> sTitel = entity.getList();
				sheet.addMergedRegion(new CellRangeAddress(index, index, cellIndex, (cellIndex + sTitel.size()) - 1));
				for (int j = 0, size = sTitel.size(); j < size; j++) {
					ExcelExportUtil.createStringCell(row1, cellIndex, sTitel.get(j).getName(), titleStyle, entity);
					cellIndex++;
				}
			} else {
	...
	public static int createHeaderRow(ExcelTitle entity, Sheet sheet, HSSFWorkbook workbook, int feildWidth) {
		...
		ExcelExportUtil.createStringCell(row, 0, entity.getTitle(), ExcelExportUtil.getHeaderStyle(workbook, entity), null);
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, feildWidth));
		if (entity.getSecondTitle() != null) {
			row = sheet.createRow(1);
			HSSFCellStyle style = workbook.createCellStyle();
			style.setAlignment(CellStyle.ALIGN_RIGHT);
			ExcelExportUtil.createStringCell(row, 0, entity.getSecondTitle(), style, null);
	 ...
	 * @return
	 */
	public static int getFieldWidth(List<ExcelExportEntity> excelParams) {
	 ...
	 * @throws Exception
	 */
	public static void getAllExcelField(String targetId, Field[] fields, List<ExcelExportEntity> excelParams, Class<?> pojoClass, List<Method> getMethods) throws Exception {
		// 遍历整个filed
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (ExcelPublicUtil.isCollection(field.getType())) {
				...
				excelEntity.setName(ExcelExportUtil.getExcelName(excel.exportName(), targetId));
				excelEntity.setOrderNum(ExcelExportUtil.getCellOrder(excel.orderNum(), targetId));
				...
				ExcelExportUtil.getExcelField(targetId, field, excelEntity, excel, pojoClass);
				if (getMethods != null) {
	 ...
	 * @throws Exception
	 */
	public static void getExcelField(String targetId, Field field, ExcelExportEntity excelEntity, Excel excel, Class<?> pojoClass) throws Exception {
		excelEntity.setName(ExcelExportUtil.getExcelName(excel.exportName(), targetId));
