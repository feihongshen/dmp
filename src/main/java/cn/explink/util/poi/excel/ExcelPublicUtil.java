package cn.explink.util.poi.excel;
...
import cn.explink.util.poi.excel.annotation.ExcelEntity;
import cn.explink.util.poi.excel.annotation.ExcelIgnore;
...
public class ExcelPublicUtil {
	...
	public static boolean isNotUserExcelUserThis(Field field, String targetId) {
		...
		if (field.getAnnotation(ExcelIgnore.class) != null) {
			boo = true;
		} else if (boo && (field.getAnnotation(ExcelCollection.class) != null) && ExcelPublicUtil.isUseInThis(field.getAnnotation(ExcelCollection.class).exportName(), targetId)) {
			boo = false;
		} else if (boo && (field.getAnnotation(Excel.class) != null) && ExcelPublicUtil.isUseInThis(field.getAnnotation(Excel.class).exportName(), targetId)) {
			boo = false;
		} else if (boo && (field.getAnnotation(ExcelEntity.class) != null) && ExcelPublicUtil.isUseInThis(field.getAnnotation(ExcelEntity.class).exportName(), targetId)) {
			boo = false;
		}
		return boo;
	}
	...
	public static Object createObject(Class<?> clazz, String targetId) {
		...
		try {
			...
			Field[] fields = ExcelPublicUtil.getClassFields(clazz);
			for (Field field : fields) {
				...
				if (ExcelPublicUtil.isCollection(field.getType())) {
					...
					setMethod = ExcelPublicUtil.getMethod(fieldname, clazz, field.getType());
					...
					fieldname = field.getName();
					setMethod = ExcelPublicUtil.getMethod(fieldname, clazz, field.getType());
					setMethod.invoke(obj, ExcelPublicUtil.createObject(field.getType(), targetId));
				}
			}
	...
	public static Method getMethod(String name, Class<?> pojoClass) throws Exception {
		StringBuffer getMethodName = new StringBuffer(ExcelPublicUtil.GET);
	...
	public static Method getMethod(String name, Class<?> pojoClass, Class<?> type) throws Exception {
		StringBuffer getMethodName = new StringBuffer(ExcelPublicUtil.SET);
		getMethodName.append(name.substring(0, 1).toUpperCase());
		getMethodName.append(name.substring(1));
