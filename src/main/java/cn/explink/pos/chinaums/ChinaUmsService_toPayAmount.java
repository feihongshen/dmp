...
public class ChinaUmsService_toPayAmount extends ChinaUmsService {
	...
	public String toPayAmountForPos(Transaction rootnote, ChinaUms chinaUms) {
		...
		try {
			...
			respNote = ExcutePosPayMethod(respNote.getCwbOrder(), respNote, rootnote); // 更新支付交易数据
	 ...
	 * @throws Exception
	 */
	private ChinaUmsRespNote ExcutePosPayMethod(CwbOrder cwbOrder, ChinaUmsRespNote respNote, Transaction rootnote) throws Exception {
		try {
			...
			if (respNote.getDeliverstate().getBusinessfee().compareTo(BigDecimal.ZERO) == 0 || cwbOrder.getCwbordertypeid() == CwbOrderTypeIdEnum.Shangmentui.getValue()) {
				ExcuteCwbSignHandler(cwbOrder, respNote.getDeliverstate(), respNote, rootnote);
				respNote.setResp_code(ChinaUmsExptMessageEnum.Success.getResp_code());
				respNote.setResp_msg(ChinaUmsExptMessageEnum.Success.getResp_msg());
				return respNote;
			} else {
				...
				try {
					...
					try {
						deliveryStateDAO.updateDelState(cwbOrder.getCwb(), DateTimeUtil.getNowTime(),
								"0".equals(rootnote.getTransaction_Body().getSignpeople()) ? cwbOrder.getConsigneename() : "他人签收", deliverystate, respNote.getDeliverid());// 设置成已反馈
						...
						posPayDAO.save_PosTradeDetailRecord(cwbOrder.getCwb(), respNote.getPodremark(), receivablefee_add, respNote.getDeliverid(), respNote.getPay_type(), respNote.getTrackinfo(),
								"", "0".equals(rootnote.getTransaction_Body().getSignpeople()) ? 1 : 2, "", 1, 1, acq_type, PosEnum.ChinaUms.getMethod(), 0, rootnote.getTransaction_Header()
										.getTermid());

						DeliveryState deliverstate = deliveryStateDAO.getActiveDeliveryStateByCwb(cwbOrder.getCwb());
						ExcuteCwbSignHandler(corder, deliverstate, respNote, rootnote);
	 ...
	 * @return
	 */
	private ChinaUmsRespNote ExcuteCwbSignHandler(CwbOrder cwbOrder, DeliveryState deliverstate, ChinaUmsRespNote respNote, Transaction rootnote) {
		try {
			...
			parameters.put("sign_man", rootnote.getTransaction_Body().getSignpeople().equals("0") ? cwbOrder.getConsigneename() : "他人签收");
			...
			posPayDAO.save_PosTradeDetailRecord(cwbOrder.getCwb(), respNote.getPodremark(), respNote.getCwbOrder().getReceivablefee().doubleValue(), respNote.getDeliverid(), respNote.getPay_type(),
					respNote.getTrackinfo(), rootnote.getTransaction_Body().getSignpeople().equals("0") ? respNote.getCwbOrder().getConsigneename() : "他人签收", rootnote.getTransaction_Body()
							.getSignpeople().equals("0") ? 1 : 2, "", 2, 1, "", PosEnum.ChinaUms.getMethod(), 0, rootnote.getTransaction_Header().getTermid());
