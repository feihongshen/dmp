...
public class AlipayUnmarchal {
	...
	public static TenPayBean UnmarchalTenPayBean(String xmlstr) throws JAXBException, UnsupportedEncodingException {
		JAXBContext jc = JAXBContext.newInstance(TenPayBean.class);
		Unmarshaller m = jc.createUnmarshaller();
		InputStream iStream = new ByteArrayInputStream(xmlstr.getBytes("UTF-8"));
		return (TenPayBean) m.unmarshal(iStream);
	}
}
