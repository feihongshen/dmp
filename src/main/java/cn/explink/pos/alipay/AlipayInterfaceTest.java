...
public class AlipayInterfaceTest {
	...
	@RequestMapping("/pay_split")
	public @ResponseBody String test_pay_split(HttpServletRequest request) {
		String cwb = request.getParameter("cwb");
		String delivery_man = request.getParameter("delivery_man");
	...
	private String XML_Search(String cwb, String delivery_man) {
		String content = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>"
				+ "<Transaction><Transaction_Header><transaction_id>MI0010</transaction_id><requester>alipay</requester>"
				+ "<target>explink</target><request_time>20121227163526</request_time><version>1.0</version>"
				+ "<ext_attributes><delivery_man>"
				+ delivery_man
				+ "</delivery_man></ext_attributes><MAC>lYCYJU/37w9HBIJw8O5jNzK2weJhl9ESoZUFpU+GHpq1SWMg1b79QrudNSGHFeAiIf13A777nZF7lYhROENAuX9QanxpX6B1T1jMp+BmgwNert+jMGmUcZAAheV5uaJTKScFMPy0pN0mqy9L9vOIUvRG/GZeGt4DmJlMs0miP+A=</MAC></Transaction_Header><Transaction_Body><order_no>"
				+ cwb + "</order_no></Transaction_Body></Transaction>";
		return content;
	}

	private String XML_payAmount(String cwb, String delivery_man, String payamount) {
		// 支付。
		String content = "<Transaction>" + "<Transaction_Header>" + "<transaction_id>MI0005</transaction_id> " + "<requester>alipay</requester> " + "<target>explink</target>"
				+ "<request_time>20101227000802</request_time> " + "<version>1.0</version><ext_attributes> " + "<delivery_dept_no>19403</delivery_dept_no> "
				+ "<delivery_dept>南京白下营业部</delivery_dept> " + "<delivery_man>"
				+ delivery_man
				+ "</delivery_man> "
				...
				+ payamount
				+ "</order_amt> "
				+ "<acq_type>single</acq_type> " + "</Transaction_Body> " + "</Transaction>";
		return content;
	}

	private String XML_payAmount_split(String cwb, String delivery_man) {
		// 支付。
		String content = "<Transaction>" + "<Transaction_Header>" + "<transaction_id>MI0005</transaction_id> " + "<requester>alipay</requester> " + "<target>explink</target>"
				+ "<request_time>20101227000802</request_time> " + "<version>1.0</version><ext_attributes> " + "<delivery_dept_no>19403</delivery_dept_no> "
				+ "<delivery_dept>南京白下营业部</delivery_dept> " + "<delivery_man>"
				...
				+ cwb
				+ "</order_no> "
				+ "<order_payable_amt>10</order_payable_amt> "
				+ "<pay_type>02</pay_type> "
				+ "<terminal_id>80040952</terminal_id>"
				+ "<trace_no>001147</trace_no> "
				+ "<alipay_trace_no>2012080200065300242901</alipay_trace_no> "
				...
				+ "<acq_type>split</acq_type> " + "</Transaction_Body> " + "</Transaction>";
		return content;
	}

	private String XML_sign(String cwb, String delivery_man) {
		// 签收
		String content = "<Transaction> " + "<Transaction_Header> " + "<transaction_id>MI0006</transaction_id> " + "<requester>alipay</requester> " + "<target>explink</target> "
				+ "<request_time>20101227001227</request_time> " + "<version>1.0</version> " + "<ext_attributes> " + "<delivery_dept_no>5361</delivery_dept_no> "
	...
	private String XML_exptFeedBack(String cwb, String delivery_man) {
		// 异常反馈
		String content = "<Transaction> " + "<Transaction_Header> " + "<transaction_id>MI0008</transaction_id>  " + "<requester>alipay</requester>  " + "<target>explink</target>  "
				+ "<request_time>20101227165134</request_time>  " + "<version>1.0</version>  " + "<ext_attributes>  " + "<delivery_dept_no>8341</delivery_dept_no>  "
				+ "<delivery_dept>上海闸北中兴路营业部</delivery_dept>  " + "<delivery_man>" + delivery_man + "</delivery_man>  " + "<delivery_name>张三丰</delivery_name>  " + "<ex_packages>1</ex_packages>  "
				+ "<logistics_alias>DEPPON</logistics_alias>  " + "</ext_attributes>  " + "<MAC>BpJf9u459cnK8Pw/1zvonWAdNzTf48iYZHFe84ZKnNHNWlNBJCfkJfYQUEUrl3V/  "
				+ "QZHZuGBWQ7Ecrmo7p9bxs9CMhgqBPz73W+AO2oabN5cvdFMnmAvw95uvBJzMmbbzq1dCm7Ewk " + "pjCnIIV3kLRwXmzaJMuLdjUZz1sf4ANRsM=</MAC>  " + "</Transaction_Header>  " + "<Transaction_Body>  "
				+ "<order_no>" + cwb + "</order_no>  " + "<ex_code>19</ex_code>  " + "<ex_desc>客户拒收</ex_desc>  " + "</Transaction_Body>  " + "</Transaction> ";
		return content;
	}

	private String XML_backOut(String cwb, String delivery_man) {
		// 撤销

		String aaa = "<Transaction>" + "<Transaction_Header>" + "<transaction_id>MI0007</transaction_id>" + "<requester>alipay</requester>" + "<target>explink</target>"
				+ "<request_time>20101227084700</request_time>" + "<version>1.0</version>" + "<ext_attributes>" + "<delivery_dept_no>8618</delivery_dept_no>" + "<delivery_dept>国贸站</delivery_dept>"
				+ "<delivery_man>" + delivery_man + "</delivery_man>" + "<delivery_name>张三</delivery_name>" + "<logistics_alias>DEPPON</logistics_alias>" + "</ext_attributes>"
				+ "<MAC>iEw4q05THxeAMfZJ71aodlYf9qXAbemyNtdgJwyds44t/"
				...
				+ "<Transaction_Body>" + "<order_no>" + cwb + "</order_no>" + "<void_amt>10</void_amt> " + "<terminal_id>80040099</terminal_id>" + "<void_trace_no>000147</void_trace_no>"
