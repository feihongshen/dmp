...
public class CwbDAO {
	...
	public List<CwbOrder> getCwbByCwbs(String cwbs) {
		...
		try {
			lc = this.jdbcTemplate.query("SELECT * from express_ops_cwb_detail where cwb in(" + cwbs + ") and state=1 ORDER BY CONVERT( consigneeaddress USING gbk ) COLLATE gbk_chinese_ci ASC",new CwbMapper());
...
*/
	...
	public List<CwbOrder> findCwbByCustomerid(long customerid){
		String sql="select * from express_ops_cwb_detail where state=1 and customerid="+customerid;

		List<CwbOrder> l;
		try {
			l = this.jdbcTemplate.query(sql, new CwbMapper());
		} catch (DataAccessException e) {
			// TODO Auto-generated catch block
			return null;
		}
	...
	public List<CwbOrder> getCwbByCwbsAndType(String cwbs,String cwbtypeid) {
		if(cwbs!=null&&!cwbs.equals("")){
				 List<CwbOrder> l=null;
				try {
					l=this.jdbcTemplate.query("SELECT * from express_ops_cwb_detail where cwb in("+cwbs+") and cwbordertypeid='"+cwbtypeid+"' and state=1 ORDER BY CONVERT( consigneeaddress USING gbk ) COLLATE gbk_chinese_ci ASC",
							new CwbMapper());
				} catch (DataAccessException e) {
					// TODO Auto-generated catch block
				return null;
	...
	public List<CwbOrder> getCwbByCwbsAndTypeByPage(String cwbs,String cwbtypeid,int start,int pageSize) {
		 List<CwbOrder> l=null;
		 try {
		l=this.jdbcTemplate.query("SELECT * from express_ops_cwb_detail where cwb in("+cwbs+") and cwbordertypeid='"+cwbtypeid+"' and state=1 ORDER BY CONVERT( consigneeaddress USING gbk ) COLLATE gbk_chinese_ci ASC limit "+start+","+pageSize,
				new CwbMapper());
		} catch (DataAccessException e) {
			// TODO Auto-generated catch block
		return null;
	...
	public List<CwbOrder> getCwbByCwbsPage(String cwbs, int start, int number) {
		List<CwbOrder> l;
		try {
			l = this.jdbcTemplate.query("SELECT * from express_ops_cwb_detail where cwb in(" + cwbs + ") and state=1 ORDER BY CONVERT( consigneeaddress USING gbk ) COLLATE gbk_chinese_ci ASC limit "+start+","+number,
					new CwbMapper());
		} catch (DataAccessException e) {
			// TODO Auto-generated catch block
			return null;
