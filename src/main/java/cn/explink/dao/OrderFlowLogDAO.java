...
public class OrderFlowLogDAO {
	 ...
	 * @return key
	 */
	public long creOrderFlow(final OrderFlow of, final String credate,final long id) {
		KeyHolder key = new GeneratedKeyHolder();
		this.jdbcTemplate.update(new PreparedStatementCreator() {
			@Override
			public PreparedStatement createPreparedStatement(java.sql.Connection con) throws SQLException {
				PreparedStatement ps = null;
				ps = con.prepareStatement("insert into express_ops_order_flow_log (floworderid,cwb,branchid,userid,floworderdetail,flowordertype,isnow,comment,credate) " + "values(?,?,?,?,?,?,1,?,?)",
						new String[] { "floworderid" });
				ps.setLong(1, id);
				ps.setString(2, of.getCwb());
				ps.setLong(3, of.getBranchid());
	...
	public long mehCreOrderFlow(final OrderFlow of, final String credate) {
		...
		this.jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(java.sql.Connection con) throws SQLException {
				PreparedStatement ps = null;
				ps = con.prepareStatement("insert into express_ops_order_flow_log (cwb,branchid,userid,floworderdetail,flowordertype,isnow,comment,credate) " + "values(?,?,?,?,?,?,?,?)",
						new String[] { "floworderid" });
	...
	public long creAndUpdateOrderFlow(OrderFlow of,long floworderid) {

		this.jdbcTemplate.update("update express_ops_order_flow_log set isnow=0 where cwb=? and isnow=1", of.getCwb());
		return this.creOrderFlow(of, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(of.getCredate()),floworderid);
	}
	...
	public List<OrderFlow> getOrderFlowListByCwb(String cwb) {
		List<OrderFlow> orderFlowList = this.jdbcTemplate.query("SELECT of_b.*,u.username FROM express_set_user u RIGHT JOIN " + "(SELECT of.*,b.branchname FROM "
				+ "(SELECT * FROM express_ops_order_flow_log WHERE cwb=?) " + "of LEFT JOIN express_set_branch b ON b.branchid=of.branchid )" + " of_b ON of_b.userid=u.userid ", new OrderFlowLogRowMapper(),
				cwb);
		return orderFlowList;
	}

	public OrderFlow getOrderCurrentFlowByCwb(String cwb) {
		String sql = "select * from express_ops_order_flow_log where cwb = ? and isnow = 1";
		Object[] paras = new Object[] { cwb };
		List<OrderFlow> orderFlowList = this.jdbcTemplate.query(sql, paras, new OrderFlowLogRowMapper());
	...
	public List<OrderFlow> getOrderFlowByFlowordertypeAndCwb(long flowordertype, String cwb, long branchid) {
		String sql = "select * from express_ops_order_flow_log where flowordertype=? and cwb=? and branchid=?";
	...
	public OrderFlow getOrderFlowByCwbAndState(String cwb, long isnow) {
		try {
			String sql = "select `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` from express_ops_order_flow_log where cwb='" + cwb + "' and isnow=" + isnow;
	...
	public OrderFlow getOrderFlowByCwbAndStateAll(String cwb, long isnow) {
		try {
			String sql = "select * from express_ops_order_flow_log where cwb='" + cwb + "' and isnow=" + isnow;
			return this.jdbcTemplate.queryForObject(sql, new OrderFlowLogRowMapper());
		} catch (Exception e) {
			return null;
		}
	}
	...
	public OrderFlow getOrderFlowById(long id) {
		String sql = "select * from express_ops_order_flow_log where floworderid = ?";
	...
	public BigDecimal[] getOrderFlowJoinCwbByBranchidAndCustomerList(long branchid, long customerid, String toDay, long flowordertype) {
		String sql = "SELECT COUNT(1) as num,SUM(of_cd.receivablefee) as receivablefee,SUM(of_cd.paybackfee) as paybackfee FROM "
				+ "(SELECT DISTINCT of.`cwb`,of.`flowordertype`,cd.receivablefee,cd.paybackfee FROM `express_ops_order_flow_log` of "
	...
	public List<OrderFlow> getOrderFlowByCwb(String cwb) {
		return this.jdbcTemplate.query("select * from express_ops_order_flow_log where cwb= ?  order by  credate ASC,flowordertype asc", new OrderFlowLogRowMapper(), cwb);
	}
	...
	public List<OrderFlow> getOrderFlowByWhere(long page, long branchid, long userid, long flowordertype, String beginemaildate, String endemaildate, long onePageNumber) {
		String sql = "select `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` from express_ops_order_flow_log ";
	...
	public List<String> getOrderFlowLingHuoList(long branchid, String flowordertypes, String beginemaildate, String endemaildate) {
		String sql = "select cwb from express_ops_order_flow_log ";
		// 强制索引
		if ((beginemaildate.length() > 0) || (endemaildate.length() > 0)) {
			sql = sql.replace("express_ops_order_flow_log", "express_ops_order_flow_log FORCE INDEX(FlowCredateIdx)");
		}
	...
	public List<String> getOrderFlowJinRiChuKuORRuKuList(final long branchid, String flowordertypes, String beginemaildate) {
		String sql = "select * from express_ops_order_flow_log FORCE INDEX(FlowCredateIdx) where credate >='" + beginemaildate + "' " + " and flowordertype in(" + flowordertypes + ")";
	...
	public long getOrderFlowCount(long branchid, long userid, long flowordertype, String beginemaildate, String endemaildate) {
		String sql = "select  count(1) from express_ops_order_flow_log ";
		sql = this.getOrderFlowByWhereSql(sql, branchid, userid, flowordertype, beginemaildate, endemaildate);
	 ...
	 * sql="select * from express_ops_order_flow_log where cwb = '"+cwb+"'";
	...
	public List<OrderFlow> getOrderFlowByCwbAndFlowordertype(long flowordertype, String cwb) {
		String sql = "select `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` from express_ops_order_flow_log  where flowordertype=? and cwb = ?";
	...
	public List<String> getOrderFlowByCredateAndFlowordertype(String begindate, String enddate, long flowordertype, final String[] operationOrderResultTypes, final String[] dispatchbranchids,
		...
		String sql = "select * from express_ops_order_flow_log FORCE INDEX(FlowCredateIdx)  where flowordertype=" + flowordertype;
	...
	public List<String> getOrderFlowBySome(String begindate, String enddate, String flowordertypes, String currentBranchids, long isnowdata) {
		String sql = "select cwb from express_ops_order_flow_log FORCE INDEX(FlowCredateIdx)  where flowordertype in(" + flowordertypes + ") " + " and credate >= '" + begindate + "'  and credate <= '"
	...
	public List<OrderFlow> getOrderFlowForZhanDianChuZhan(String begindate, String enddate, final String[] startbranchids, final String[] nextbranchids, final long flowordertype) {
		String sql = "select * from express_ops_order_flow_log FORCE INDEX(FlowCredateIdx) where credate >= '" + begindate + "' ";
		StringBuilder deliverystatesql = new StringBuilder();
		sql += " and credate <= '" + enddate + "' ";
	...
	public List<OrderFlow> getAdvanceOrderFlowByCwb(String cwb) {
		return this.jdbcTemplate.query("select `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` from express_ops_order_flow_log where cwb= '" + cwb
	...
	public List<OrderFlow> getOperateOrderFlowByWhere(long page, String branchids, long flowordertype, String beginemaildate, String endemaildate, String autodetailflag) {
		String sql = "select * from express_ops_order_flow_log ";
	...
	public List<String> getOneCwbs(String flowordertypes, String begindate, String enddate, String kufangids, String branchids) {
		String sql = "SELECT DISTINCT d.cwb FROM `express_ops_order_flow_log` AS f FORCE INDEX(FlowCredateIdx) LEFT JOIN `express_ops_cwb_detail` AS d ON f.cwb=d.cwb " + "WHERE f.flowordertype in("
	...
	public List<String> getTwoCwbs(String flowordertypes, String cwbs, String enddate) {
		String sql = "SELECT DISTINCT cwb FROM express_ops_order_flow_log WHERE cwb IN(" + cwbs + ")" + " AND flowordertype in(" + flowordertypes + ") AND credate>? ";
	...
	public List<String> getThreeCwbs(String flowordertypes, String cwbs, String noIncwbs) {
		String sql = "SELECT DISTINCT cwb FROM express_ops_order_flow_log WHERE cwb IN(" + cwbs + ")" + " AND cwb NOT IN(" + noIncwbs + ") AND flowordertype in(" + flowordertypes + ") ";
		return this.jdbcTemplate.queryForList(sql, String.class);
	}
	...
	public long getFiveCwbsCount(String flowordertypes, String cwbs, String noIncwbs) {
		String sql = "SELECT count(DISTINCT cwb) FROM express_ops_order_flow_log WHERE cwb IN(" + cwbs + ")  " + "AND cwb NOT IN(" + noIncwbs + ") AND flowordertype in(" + flowordertypes + ") ";
	...
	public long getOrderFlowByCredateAndFlowordertypeCount(long branchid, String begindate, String enddate, long flowordertype, long deliverid) {
		String sql = "select count(1) from express_ops_order_flow_log  where flowordertype=" + flowordertype + " and isnow=1";
		StringBuilder deliverystatesql = new StringBuilder();
	...
	public List<OrderFlow> getCwbsByFlowordertype(FlowOrderTypeEnum flowordertype, String cwbs) {
		String sql = "SELECT `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` FROM express_ops_order_flow_log WHERE cwb IN(" + cwbs + ")" + " AND flowordertype="
	...
	public List<OrderFlow> getCwbsByDateAndFlowtype(String emailStartTime, String eamilEndTime, long flowordertype) {
		String sql = "SELECT  `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` FROM express_ops_order_flow_log FORCE INDEX(FlowCredateIdx) WHERE flowordertype =? "
	...
	public List<String> getZaituCwbsByDateAndFlowtype(String emailStartTime, String eamilEndTime, long flowordertype, String branchids) {
		String sql = "SELECT  cwb FROM express_ops_order_flow_log FORCE INDEX(FlowCredateIdx) WHERE flowordertype =? " + " AND credate>=? AND credate<=? and branchid in (" + branchids + ") and isnow=1 ";
	...
	public List<String> getIntoCwbDetailAndOrderFlow(String begindate, String enddate, long flowordertype, long branchid) {
		String sql = "select cwb from express_ops_order_flow_log where flowordertype=" + flowordertype + " and credate >= '" + begindate + "' and credate <= '" + enddate + "' ";
	...
	public String getCwbDetailAndOrderFlowByParamSQL(long page, String customers, String emaildatebegin, String emaildateend, String begindate, String enddate, long flowordertype, long branchid,
			long cwbordertypeid) {
		String sql = "select cd.* from express_ops_cwb_detail cd left join express_ops_order_flow_log of on cd.cwb=of.cwb" + " where cd.state=1 and of.flowordertype=" + flowordertype
	...
	public OrderFlow getOrderFlowByParam(long flowordertype, String cwb) {
		try {
			String sql = "select * from express_ops_order_flow_log where flowordertype=? and cwb=? order by  credate DESC";
	...
	public OrderFlow getOrderFlowByIsnow(String cwb) {
		try {
			String sql = "select * from express_ops_order_flow_log where isnow=1 and cwb='" + cwb + "' ";
	...
	public List<OrderFlow> getOrderFlowByCwbs(List<String> cwbs) {
		StringBuilder stringBuilder = new StringBuilder("select `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` from express_ops_order_flow_log where cwb in  (");
		for (String cwb : cwbs) {
	...
	public List<OrderFlow> getCwbByFlowordertypeAndCwbs(int value, String cwbs) {
		if (cwbs.trim().length() > 0) {
			String sql = " select `cwb`,`branchid`,`credate`,`userid`,`flowordertype`,`isnow`,`outwarehouseid`,`comment` from express_ops_order_flow_log where flowordertype=?  and cwb in(" + cwbs
	...
	public OrderFlow getOrderFlowByCwbAndFlowtype(String cwb, String flowordertypes) {
		try {
			String sql = "select * from express_ops_order_flow_log where  cwb=? and flowordertype in(" + flowordertypes + ") order by credate desc limit 0,1";
	...
	public OrderFlow queryFlow(String cwb, FlowOrderTypeEnum flowType) {
		try {
			String sql = "select * from express_ops_order_flow_log where cwb= ? and flowordertype = ? order by credate desc limit 1";
			return this.jdbcTemplate.queryForObject(sql, new OrderFlowLogRowMapper(), cwb, flowType.getValue());
		} catch (DataAccessException e) {
			return null;
		}
	}
	...
	public void updateCommentByCwb(String cwb, String comment) {
		String sql = "update express_ops_order_flow_log set comment=? where cwb=? and isnow=1 ";
	...
	private void batchInsertOrderFlow(String[] cwbs, long reportOutAreaBranchId, long reportOutAreaUserId) {
		...
		sql.append("insert into express_ops_order_flow_log(cwb , branchid , credate , userid,flowordertype ,isnow)values(? , ");
	...
	private void batchRestNowFlow(String[] cwbs) {
		String sql = "update express_ops_order_flow_log set isnow = 0 where cwb in (" + this.getInPara(cwbs) + ")";
	...
	public Map<String, Object> queryOrderFlowAndCredate(String cwb) {
		String sql = "select credate,flowordertype from express_ops_order_flow_log where cwb = '" + cwb + "' and isnow = 1";
	...
	public void deleteOrderFlow(String cwb, FlowOrderTypeEnum... flows) {
		...
		String sql = "delete from express_ops_order_flow_log where cwb = ? and " + this.getFlowOrderTypeCond(flows);
	...
	public void setOrderCurrentFlow(String cwb, FlowOrderTypeEnum flow) {
		String resetSql = "update express_ops_order_flow_log set isnow = 0 where cwb = ? and isnow = 1";
		this.jdbcTemplate.update(resetSql, cwb);
		String setSql = "update express_ops_order_flow_log set isnow = 1 where cwb = ? and flowordertype = " + flow.getValue();
