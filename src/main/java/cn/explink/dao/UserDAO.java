...
public class UserDAO {
	...
	public User getUserByUsername(String username) {
		return jdbcTemplate.queryForObject("SELECT * from express_set_user where username=? and userDeleteFlag=1", new UserRowMapper(), username);
	}

	public List<User> getUsersByUsernameLogin(String username) {
		List<User> userList = jdbcTemplate.query("SELECT * from express_set_user where username=? and userDeleteFlag=1 and employeestatus<>3", new UserRowMapper(), username);
		return userList;
	}
	...
	public User getUserByUserid(long userid) {
		User user = new User();
		try {
			user = jdbcTemplate.queryForObject("SELECT * from express_set_user where userid=? and userDeleteFlag=1 ", new UserRowMapper(), userid);
		} catch (Exception e) {
			user = new User();
		}
		return user;
	...
	public List<User> getUserByid(long userid) {
		return jdbcTemplate.query("SELECT * from express_set_user where userid=? and userDeleteFlag=1 ", new UserRowMapper(), userid);
	}

	public User getAllUserByid(long userid) {
		try {
			return jdbcTemplate.queryForObject("SELECT * from express_set_user where userid=?", new UserRowMapper(), userid);
	...
	public List<User> getUserByRole(int roleid) {
		...
		List<User> userList = jdbcTemplate.query(sql, new UserRowMapper(), roleid);
		return userList;
	}
	...
	public List<User> getUserByRole(String roleids, long branchid) {
		...
		List<User> userList = jdbcTemplate.query(sql, new UserRowMapper(), branchid);
		return userList;
	}

	public User getUsersByRealnameAndRole(String realname, int roleid) {
		try {
			User user = jdbcTemplate.queryForObject("SELECT * from express_set_user where realname=? and roleid=? and userDeleteFlag=1 and employeestatus=1", new UserRowMapper(), realname, roleid);
			return user;
	...
	public User getUsersByRealnameAndRole(String realname, String roleids) {
		try {
			...
			User user = jdbcTemplate.queryForObject(sql, new UserRowMapper(), realname);
			return user;
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	...
	public List<User> getUserByRoleAndBranchid(int roleid, long branchid) {
		try {
			...
			return jdbcTemplate.query(sql, new UserRowMapper(), roleid, branchid);
	...
	public List<User> getAllUserByRolesAndBranchid(String roleids, long branchid) {
		try {
			...
			return jdbcTemplate.query(sql, new UserRowMapper(), branchid);
		} catch (EmptyResultDataAccessException ee) {
			return null;
		}
	}
	...
	public User getSingelUsersByRealname(String realname) {
		try {
			...
			return jdbcTemplate.queryForObject(sql, new UserRowMapper(), realname);
		} catch (EmptyResultDataAccessException ee) {
			return null;
		}
	...
	public String getDeliveryUserNameByCwb(String cwb) {
		...
		try {
			...
			deliverid = jdbcTemplate.queryForLong(sql, cwb);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (deliverid != 0) {
			String usersql = "select username from express_set_user where userid=" + deliverid;
			userMap = jdbcTemplate.queryForMap(usersql);
			username = userMap.get("username").toString();
		}
		logger.info("POS部分查询订单号cwb:" + cwb + ",username:" + username);
		return username;
	...
	public void updateUserCode(String deliverManCode, long userid) {
		...
		jdbcTemplate.update(sql, deliverManCode, userid);
	}
	...
	public User getUserByUseridLock(long userid) {
		return jdbcTemplate.queryForObject("SELECT * from express_set_user where userid=? for update ", new UserRowMapper(), userid);
	}
	...
	public void updateUserAmount(long userid, BigDecimal deliverAccount, BigDecimal deliverPosAccount) {
		...
		jdbcTemplate.update(sql, deliverAccount, deliverPosAccount, userid);
	}
	...
	public List<User> getDeliverAccountList(long page, long branchid, String realname, String deliverAccountStartStr, String deliverAccountEndStr, String deliverPosAccountStartStr,
		...
		sql += getDeliverAccountListWhere(branchid, realname, deliverAccountStartStr, deliverAccountEndStr, deliverPosAccountStartStr, deliverPosAccountEndStr, uee);
	...
	public JSONObject getAllGruopNumWorker() {
		...
		JSONObject us = (JSONObject) jdbcTemplate.query(sql, new ResultSetExtractor() {
	...
	public List<User> getUserByUsernameAndMobile(String mobile) {
		try {
			...
			return jdbcTemplate.query(sql, new UserRowMapper(), mobile);
		} catch (EmptyResultDataAccessException ee) {
			return null;
		}
	}
	...
	public void updateUserLastInfo(User user) {
		...
		jdbcTemplate.update(sql, user.getLastLoginIp(), user.getLastLoginTime(), user.getUserid());
	}
	...
	public Object getAllUserByWorkState(int status) {
		return jdbcTemplate.queryForLong(" select count(1) from  express_set_user where userDeleteFlag=1 and employeestatus=" + status);
	}
	...
	public User getUserByBranchName(long branchid) {
		return jdbcTemplate.queryForObject("SELECT * from express_set_user where branchid=? and userDeleteFlag=1 limit 1 ", new UserRowMapper(), branchid);
	}
	...
	public Map<Long, String> getUserNameMap(List<Long> idList) {
		...
		String sql = "select userid , username from express_set_user where userid in (" + sqlInParam + ")";
