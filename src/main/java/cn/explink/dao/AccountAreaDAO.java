...
public class AccountAreaDAO {
	...
	public List<AccountArea> getAccountAreaByCustomerid(long customerid) {
		String sql = "select * from express_set_account_area where customerid=? and isEffectFlag=1";
	...
	public List<AccountArea> getAccountAreaByPage(long page, long customerid) {
		String sql = "select aa.* from express_set_account_area aa RIGHT JOIN (SELECT customerid,ifeffectflag FROM  express_set_customer_info WHERE ifeffectflag=1 ) ci ON ci.customerid=aa.customerid ";
		sql = this.getAccountAreaByPageWhereSql(sql, customerid);
		sql += " order by aa.isEffectFlag desc limit " + (page - 1) * Page.ONE_PAGE_NUMBER + " ," + Page.ONE_PAGE_NUMBER;
	...
	private String getAccountAreaByPageWhereSql(String sql, long customerid) {
		sql += " where aa.isEffectFlag is not null ";
		if (customerid >= 0) {
			sql += " and aa.customerid=" + customerid;
		}
		return sql;
	}

	public long getAccountAreaCount(long customerid) {
		String sql = "SELECT count(1) from express_set_account_area  aa RIGHT JOIN (SELECT customerid,ifeffectflag FROM  express_set_customer_info WHERE ifeffectflag=1 ) ci ON ci.customerid=aa.customerid ";
		sql = this.getAccountAreaByPageWhereSql(sql, customerid);
	...
	public List<AccountArea> getAccountAreaByIsEffectFlag() {
		return jdbcTemplate.query("select * from express_set_account_area where isEffectFlag =1", new AccountAreaRowMapper());
	}

	public void delAccountArea(long customerid) {
		jdbcTemplate.update("update express_set_account_area set isEffectFlag=(isEffectFlag+1)%2 where customerid=? ", customerid);
	}
}
