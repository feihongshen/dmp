...
public class ExpressSysMonitorDAO extends BasicJdbcTemplateDaoSupport<ExpressSysMonitor, Long> {
	...
	public void updateDealFlag(ExpressSysMonitor expressSysMonitor) {
		String sql = "UPDATE express_sys_monitor SET dealflag = 1 WHERE dealflag = 0 AND type = ? AND optime <= ?";
		try {
			getJdbcTemplate().update(sql, expressSysMonitor.getType(), expressSysMonitor.getOptime());
