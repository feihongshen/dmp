...
public class AbnormalTypeDAO {
	...
	public List<AbnormalType> getAllAbnormalTypeByName() {
		...
		return this.jdbcTemplate.query(sql, new AbnormalTypeRowMapper());
	}
	...
	public List<AbnormalType> getAbnormalTypeByName(String name) {
		...
		return this.jdbcTemplate.query(sql, new AbnormalTypeRowMapper(), name);
	}
	...
	public AbnormalType getAbnormalTypeById(long id) {
		try {
			...
			return this.jdbcTemplate.queryForObject(sql, new AbnormalTypeRowMapper(), id);
		} catch (Exception e) {
			return null;
		}
	}
	...
	public void delAbnormalType(long id) {
		this.jdbcTemplate.update("update express_set_abnormal_type set state=(state+1)%2 where id=?", id);
	}
