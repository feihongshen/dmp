 ...
 */
@Repository
public class ExpressStationFreightAuditDao {
	...
	private final class ExpressFreightBillRowMapper implements RowMapper<ExpressFreightAuditBillVO>{
		...
		public ExpressFreightAuditBillVO mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			expressFreightBill.setPayMethod(ExpressSettleWayEnum.getByValue(rs.getInt("pay_method")).getText());
			expressFreightBill.setBranchId(rs.getLong("branch_id"));
			expressFreightBill.setBranchName(rs.getString("branch_name"));
	...
	public List<ExpressFreightAuditBillVO> queryBillForFreight(Long page,ExpressFreightAuditBillVO expressFreightAuditBillVO){
		String sql = "SELECT * FROM express_ops_freight_bill where 1=1 and bill_type="+ExpressBillTypeEnum.StationFreightBill.getValue();//1表示账单类型为站点运费账单
	...
	public int getBillCount(ExpressFreightAuditBillVO expressFreightAuditBillVO){
		String sql = "select count(1) from express_ops_freight_bill where 1=1  and bill_type="+ExpressBillTypeEnum.StationFreightBill.getValue();//1表示账单类型为站点运费账单
		sql += getConditionsSql(expressFreightAuditBillVO);
		if(expressFreightAuditBillVO.getSortRule()==null){
			return 0;
		}else{
	...
	public String getConditionsSql(ExpressFreightAuditBillVO expressFreightAuditBillVO){
		...
		//站点
		if(!Tools.isEmpty(expressFreightAuditBillVO.getBranchId())){
			sql.append(" and branch_id ='"+expressFreightAuditBillVO.getBranchId()+"'");
		}
		//排序
