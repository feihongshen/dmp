 ...
 */
@Repository
public class ExpressCustomerFreightAuditDao {
	...
	public List<ExpressFreightAuditBillVO> queryBillForFreight(Long page,ExpressFreightAuditBillVO expressFreightAuditBillVO){
		String sql = "SELECT * FROM express_ops_freight_bill where 1=1 and bill_type="+ExpressBillTypeEnum.CustomerFreightBill.getValue();//0表示账单类型为客户运费账单
	...
	public int getBillCount(ExpressFreightAuditBillVO expressFreightAuditBillVO){
		String sql = "select count(1) from express_ops_freight_bill where 1=1 and bill_type="+ExpressBillTypeEnum.CustomerFreightBill.getValue();//0表示账单类型为客户运费账单
		sql += getConditionsSql(expressFreightAuditBillVO);
		if(expressFreightAuditBillVO.getSortRule()==null){
			return 0;
		}else{
	...
	public String getConditionsSql(ExpressFreightAuditBillVO expressFreightAuditBillVO){
		...
		//客户
		if(!Tools.isEmpty(expressFreightAuditBillVO.getCustomreId())){
			sql.append(" and customer_id ='"+expressFreightAuditBillVO.getCustomreId()+"'");
		}
		//排序
