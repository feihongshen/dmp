...
/**
 * 跨省到付运费审核（应付）
 * @author wangzy 2015年8月20日
 *
 */
@Repository
public class ExpressAcrossProvincePayFreightAuditDao {
	...
	public List<ExpressFreightAuditBillVO> queryBillForFreight(Long page,ExpressFreightAuditBillVO expressFreightAuditBillVO){
		String sql = "SELECT * FROM express_ops_freight_bill where 1=1 and bill_type=" + ExpressBillTypeEnum.AcrossProvincePayableBill.getValue();//2表示账单类型为跨省应收运费账单
	...
	public int getBillCount(ExpressFreightAuditBillVO expressFreightAuditBillVO){
		String sql = "select count(1) from express_ops_freight_bill where 1=1 and bill_type=" + ExpressBillTypeEnum.AcrossProvincePayableBill.getValue();//2表示账单类型为跨省应收运费账单
		sql += getConditionsSql(expressFreightAuditBillVO);
		if(expressFreightAuditBillVO.getSortRule()==null){
			return 0;
		}else{
