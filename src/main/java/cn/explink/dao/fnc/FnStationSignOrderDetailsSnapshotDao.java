 ...
 */
@Repository
public class FnStationSignOrderDetailsSnapshotDao {

	@Autowired
	private JdbcTemplate jdbcTemplate;
	
	private Logger logger = LoggerFactory.getLogger(FnStationSignOrderDetailsSnapshotDao.class);
	...
	* @author 刘武强
	...
	public long getReportIdByCwbAndReportdate(String cwb, int reportdate) {
		String sql = "select count(1) from fn_station_sign_order_details_snapshot where cwb = ? and reportdate = ? ";
		return this.jdbcTemplate.queryForLong(sql, cwb, reportdate);
	}
	
	@DataSource(DatabaseType.REPLICA)
	public long getOrderCountByCwb(String cwb) {
		String sql = "select count(1) from fn_station_sign_order_details_snapshot where cwb = ?";
