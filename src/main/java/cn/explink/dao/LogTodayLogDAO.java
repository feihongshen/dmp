...
public class LogTodayLogDAO {
	...
	public List<String> getCwbTodaybyOrtherBranchid(long branchid, long flowordertype, String startTime, String endTime) {
		return jdbcTemplate.query("SELECT DISTINCT cwb as cwb FROM express_ops_order_flow_log FORCE INDEX(FlowCredateIdx) WHERE  " + " flowordertype=? " + " AND credate >=? " + " AND credate <=? "
	...
	public long getLousaoyidaozhanCount(long flowordertype, long branchid, String startTime) {
		String sql = "SELECT COUNT(DISTINCT cd.cwb) as num  " + "FROM  express_ops_cwb_detail cd LEFT JOIN  express_ops_order_flow_log of ON cd.`cwb`=of.`cwb` "
	...
	public long getTodaybyBranchidAndIsnow(long branchid, long flowordertype, String startTime, String endTime) {
		return jdbcTemplate.queryForLong("SELECT COUNT(1) FROM express_ops_order_flow_log WHERE  " + " flowordertype=? " + " AND credate >=? " + " AND credate <=? " + " AND branchid=? and isnow =1 ",
	...
	public List<String> getTodayWeidaohuoCwbbyChukuNextbranchidCwb(long branchid, long flowordertype) {
		return jdbcTemplate.query("  SELECT DISTINCT cd.cwb as cwb FROM express_ops_cwb_detail cd LEFT JOIN  express_ops_order_flow_log of ON cd.cwb=of.cwb  "
	...
	public long getTodayWeidaohuobyNextbranchidIsnowCount(long branchid, long flowordertype) {
		return jdbcTemplate.queryForLong(" SELECT COUNT(DISTINCT cd.cwb) FROM express_ops_cwb_detail cd LEFT JOIN  express_ops_order_flow_log of ON cd.cwb=of.cwb  "
	...
	public long getOrderByFlowtypeCount(long branchid, long flowordertype, long deliverystate, String startTime, String endTime) {
		return jdbcTemplate.queryForLong("SELECT count(DISTINCT cwb) FROM express_ops_order_flow_log FORCE INDEX(FlowCredateIdx) WHERE " + " branchid = ? " + " AND flowordertype=? " + " AND credate >=? "
