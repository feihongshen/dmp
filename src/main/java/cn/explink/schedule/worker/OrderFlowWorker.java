...
@Service("orderFlowWorker")
public class OrderFlowWorker extends ScheduledWorker {

	private Logger logger = LoggerFactory.getLogger(OrderFlowWorker.class);

	@Autowired
	private OrderFlowNestedTransactionWrapper wrapper;

	@Override
	protected boolean doJob(ScheduledTask scheduledTask) throws Exception {
		try {
			String result = (String) wrapper.doSomething(scheduledTask);
			...
			logger.error("order flow nested transaction failed du to " + e.getMessage(), e);
			return false;
		}
	}
