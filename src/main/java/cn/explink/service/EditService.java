...
public class EditService {
	...
	public void deleteEditTwo(@Header("loseCwbByEmaildateid") String parm, @Header("MessageHeaderUUID") String messageHeaderUUID) {
		...
		try {
			...
			// 把未完成MQ插入到数据库中, start
			//消费MQ异常表
			this.mqExceptionDAO.save(MqExceptionBuilder.getInstance().buildExceptionCode(this.getClass() + "deleteEditTwo")
	...
	public void deleteEdit(@Header("losecwbbatch") String cwb, @Header("MessageHeaderUUID") String messageHeaderUUID) {
		...
		try {
			...
			// 把未完成MQ插入到数据库中, start
			//消费MQ异常表
			this.mqExceptionDAO.save(MqExceptionBuilder.getInstance().buildExceptionCode(this.getClass() + "deleteEdit")
	...
	public void saveEdit(@Header("orderFlow") String parm, @Header("MessageHeaderUUID") String messageHeaderUUID) {
		try {
			...
			// 把未完成MQ插入到数据库中, start
			//消费MQ异常表
			this.mqExceptionDAO.save(MqExceptionBuilder.getInstance().buildExceptionCode(this.getClass() + "saveEdit")
