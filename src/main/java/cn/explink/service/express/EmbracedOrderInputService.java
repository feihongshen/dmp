		...
		if (StringUtils.isBlank(inputOrEmbrace) || !("input".equals(inputOrEmbrace) || "inbrace".equals(inputOrEmbrace))) {
			...
			try {
				...
				if (!"input".equals(inputOrEmbrace)) {// 只有补录才有的数据回写
					...
					// 商品数量
					doReq.setTotalNum(Integer.parseInt(StringUtils.isEmpty(embracedOrderVO.getGoods_number())?"0" : embracedOrderVO.getGoods_number().trim()));//
				...
				// 总件数				
				doReq.setTotalBox(Integer.parseInt(StringUtils.isEmpty(embracedOrderVO.getNumber()) ? "0" : embracedOrderVO.getNumber().trim()));
