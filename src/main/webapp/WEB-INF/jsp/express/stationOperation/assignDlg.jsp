...
<div id="box_contant">
	...
	<div id="box_in_bg">
		...
		<table>
			<tr>
				<td><input type="button" value="分配" class="button" onclick="doAssign();" /></td>
				<td>
					<form id="exportExcelForm">
						<input id="assignAndExportButton" type="button" value="分配并导出" class="button" onclick="assignAndExport();"/>
						<input type="hidden" id="selectedPreOrders" name="selectedPreOrders" value="<%=selectedPreOrders%>" />
					</form></td>
				<td><input type="button" value="关闭" class="button" onclick="closeBox()" /></td>
			</tr>
		</table>
	</div>
</div>
...
<!-- 导出的ajax地址 -->
<input type="hidden" id="export" value="<%=request.getContextPath()%>/stationOperation/exportExcel" />
