...
<script src="<%=request.getContextPath()%>/js/express/customerfreight/customerfreightAudit.js" type="text/javascript"></script>
</head>
...
<body style="background: #eef9ff">
			...
			<table style="width: 100%">
				<tr>
					<td><input class="input_button2" type="button"
						id="confirm_btn" onclick="edit('customerAuditTable');"
						value="查看/修改" /> <input class="input_button2" type="button" id="query"
						value="查询" /></td>
				</tr>
			</table>
		</div>
		...
		<div class="right_title">
			<div style="overflow: auto;">
				<table width="100%" border="0" cellspacing="1" cellpadding="0"
					class="table_2" id="customerAuditTable">
					<tr>
					...
					<c:forEach items="${billList}" var="list">
						<tr>
							<td height="30px" align="center" valign="middle"><input
								type="checkbox" id="${list.id}" name="checkBoxx" value="${list.id}"
								onclick="initColor(this);" /></td>
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="0"
			...
			<tr>
				<td height="38" align="center" valign="middle" bgcolor="#eef6ff">
					<a
					href="javascript:$('#queryCondition').attr('action','${pageContext.request.contextPath}/customerFreightAudit/toCustomerFreightAudit/1');$('#queryCondition').submit();">第一页</a>
					<a
					href="javascript:$('#queryCondition').attr('action','${pageContext.request.contextPath}/customerFreightAudit/toCustomerFreightAudit/${page_obj.previous<1 ? 1 : page_obj.previous }');$('#queryCondition').submit();">上一页</a>
					<a
					href="javascript:$('#queryCondition').attr('action','${pageContext.request.contextPath}/customerFreightAudit/toCustomerFreightAudit/${page_obj.next<1 ? 1 : page_obj.next }');$('#queryCondition').submit();">下一页</a>
					<a
					href="javascript:$('#queryCondition').attr('action','${pageContext.request.contextPath}/customerFreightAudit/toCustomerFreightAudit/${page_obj.maxpage<1 ? 1 : page_obj.maxpage }');$('#queryCondition').submit();">最后一页</a>
					共${page_obj.maxpage}页 共${page_obj.total}条记录 当前第 <select
					id="selectPg"
					onchange="$('#queryCondition').attr('action',$(this).val());$('#queryCondition').submit()">
	...
	<!-- 点击查询按钮的查询条件弹出框 -->
	<div>
		<div id="QueryExpressBox" class="easyui-dialog"
			style="width: 800px; height: 250px; top: 200px; left: 200px; padding: 10px 20px; z-index: 1000px;"
			closed="true" buttons="#dlg2Match">
			<form id="queryCondition" method="post"
				action="<%=request.getContextPath()%>/customerFreightAudit/toCustomerFreightAudit/1">
				<div style="margin-left: 20px;" id="dispatchItem">
					<table>
						<tr>
							...
							<td>账单状态</td>
							<td><select id="billState" name="billState"
								style="width: 210px">
						...
						<tr>
							<td>账单创建日期</td>
							<td><input id="startCreatTime" name="startCreatTime"
								class="easyui-datebox" style="width: 100px" />至<input
								id="endCreatTime" name="endCreatTime" class="easyui-datebox"
								style="width: 100px" /></td>
							<td>账单核销日期</td>
							<td><input id="startAuditTime" name="startAuditTime"
								class="easyui-datebox" style="width: 100px" />至<input
								id="endAuditTime" name="endAuditTime" class="easyui-datebox"
								style="width: 100px" /></td>
						</tr>
						<tr>
							<td>客户</td>
							<td><select id="customreId" name="customreId"
								style="width: 210px">
							</select></td>
							<td>排序</td>
							<td><select id="sortOption" name="sortOption"
								style="width: 180px">
							...
							</select> <select id="sortRule" name="sortRule">
			...
			<div id="dlg2Match" style="text-align: center;">
				<input type="button" value="查询" id="confirmQuery"
					class="btn btn-default"  /> <input
					type="button" value="关闭" id="cancleQuery" class="btn btn-default"
					 />
			</div>
		</div>
		<!-- 点击查看编辑按钮的弹出框 -->
		<div>
			<div id="EditExpressBox" class="easyui-dialog"
				style="width: 900px; height: 450px; top: 100px; left: 200px; padding: 10px 20px; z-index: 1000px;"
				closed="true" buttons="#dlg2Match">
				<div style="margin-left: 20px;" id="dispatchItem">
					<div class="right_box">
						<div class="inputselect_box" style="width: 97%">
							<form id="queryCondition" method="post"
								action="<%=request.getContextPath()%>/expressIntoStation/expressQueryList/1">

								<table style="width: 100%">
									<tr>
										<td><input class="input_button2" type="button"
											id="returnButton" value="返回" /> <input class="input_button2"
											type="button" id="auditButton" value="审核" /> <input
											class="input_button2" type="button" id="cancalAuditButton"
											value="取消审核" /> <input class="input_button2" type="button"
											id="checkAuditButton" value="核销" /></td>
									</tr>
								</table>
								<table style="height: 200px;">
									...
									<tr>
										<td>客户</td>
										<td><input id="customerNameEdit" disabled=true /></td>
										<td>运费金额合计</td>
									...
									<tr>
										<td>备注</td>
										<td colspan="5"><textarea id="remarkEdit" disabled=true
						...
						<div class="right_title" style="width: 97%;">
							<div style="overflow: auto;">
								<table width="100%" border="0" cellspacing="1" cellpadding="0"
									class="table_2" id="customerAuditEdit"
				...
				<div id="dlg2Match" style="text-align: center;"></div>
			</div>
		<!-- 存储查询条件的隐藏域 -->
		<div>
			...
			<input type="hidden" id="customreIdValue" value="${expressFreightAuditBillVO.customreId}" /> 
