...
<body>
	...
	<div id="insertBox" class="easyui-dialog" title="新增"  style="width: 1000px; height: 450px; top:100px;left:100px; padding: 10px 10px;z-index:400px;" closed="true">
		<form id="insertForm" action="" method="post" style="margin: 10px;" >	
			<div id="alertconText"></div>
		</form>
	</div>
	
<input type="hidden" id="fatherURL" value="<%=request.getContextPath()%>/transProvincialPayableReconciliationController/" /> 
...
function updateOrSeeClick(){
	...
	if($.trim(radio_id.parent().parent().find("td").eq(2).attr('value')) != <%=ExpressBillStateEnum.UnAudit.getValue()%>){
		//$("#updateBox_remark_id").css('background','gray');
		$("#updateBox_remark_id").attr("disabled",true);
		$("#updateBox_save").hide();
	}else{
		//$("#updateBox_remark_id").css('background','#ffffff');
...
function creatClick(page){
	...
	var alerturl = $("#fatherURL").val()+"alertInfo/"+page;
	$.ajax({
		type : "POST",
		url : alerturl,
		async: false,  
...
function importClick(){
	//TODO 导入逻辑，后台预留了接口 exportInfo(),导入只负责把数据存到数据库，返回一个成功与否的标志（失败的话还需要回传错误信息）,弹出文件选择等，待完善
	var alerturl = $("#fatherURL").val()+"exportInfo";
	$.ajax({
		type : "POST",
		url : alerturl,
		async: false,  
