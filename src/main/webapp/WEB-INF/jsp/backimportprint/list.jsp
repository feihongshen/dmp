...
<%@page import="cn.explink.domain.Backintowarehouse_print"%>
			...
			function() {
				$("#find").click(
						function() {
							...
							if ($("#begincredate").val() > $("#endcredate")
									.val()) {
								alert("开始时间不能大于结束时间！");
								return false;
							}
		...
		<div class="inputselect_box">
			<form action="1" method="post" id="searchForm">
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					style="height: 30px">
					<tr>
						<td align="left">上一站:： <select name="branchid" id="branchid"
						...
						for (int i = 0; i < branchArrlist.size(); i++) {
							if (b.getBranchid() == new Long(branchArrlist
									.get(i).toString())) {%>
							...
							操作时间： <input type="text" name="begincredate" id="begincredate"
							value="<%=request.getParameter("begincredate") == null ? ""
					: request.getParameter("begincredate")%>" />
							到 <input type="text" name="endcredate" id="endcredate"
							value="<%=request.getParameter("endcredate") == null ? "" : request
					.getParameter("endcredate")%>" />
							驾驶员: <select name="driverid" id="driverid">
								...
								<option value="<%=driver.getUserid()%>"
									...
									selected="selected" <%}%>><%=driver.getRealname()%></option>
								<%
									}
								%>
						...
						打印模版:<select id="printtype" name="printtype">
						 <option value="0">请选择</option>
						 <option value="1">退货出站</option>
						 </select>
						 
						 <input type="button" id="print" value="打印" onclick="bdprint()" class="input_button2" />
							<input name="flag" id="flag" value="<%=flag%>" type="hidden" /> 
							<a id="history" href="javascript:isshow(1)">>>历史打印列表</a>
							<a id="no" href="javascript:isshow(0)">>>返回未打印打印列表</a>
						</td>
					</tr>
				</table>
			</form>
			...
			<table id="table1" width="100%" border="0" cellspacing="1" cellpadding="0"
				class="table_2">
				<tr class="font_1">
				...
				%>
				<tr>
					<td width="10%" align="center" valign="middle"><input
						id="isprint" name="isprint" type="checkbox"
						value="<%=pv.getCwb()%>" checked="checked" /><%=i%></td>
