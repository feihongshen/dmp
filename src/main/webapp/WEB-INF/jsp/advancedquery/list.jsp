...
<script type="text/javascript">
	$(document).ready(function() {
	   ...
	   $("#operationOrderResultType").multiSelect({ oneOrMoreSelected: '*',noneSelected:'请选择操作结束后的配送结果' });
	   //获取下拉框的值
	   $("#find").click(function(){
	         var checkval="";
		...
		success:function(json){
			...
			for(var j = 0 ; j < json.length ; j++){
				if(json[j].employeestatus==<%=UserEmployeestatusEnum.LiZhi.getValue()%>){
					$("<option value='"+json[j].userid+"'>"+json[j].realname+"(离职)</option>").appendTo("#dispatchdeliveryid");
				}else{
...
<div class="right_box">
		...
		<td align="left">
		   ...
		   <select name ="customerwarehouseid" id ="customerwarehouseid">
			...
			<select name ="startbranchid" id ="startbranchid" >
			 ...
			 <select name ="nextbranchid" id ="nextbranchid" >
		          <option value ="-1">请选择下一站</option>
		          <%if(branchlist!=null && branchlist.size()>0){ %>
		          <%for(Branch c : branchlist){ %>
		          <option value =<%=c.getBranchid() %> 
			...
			<select name ="currentbranchid" id ="currentbranchid" >
		          <option value ="-1">请选择当前站点</option>
		          <%if(branchlist!=null && branchlist.size()>0){ %>
		          <%for(Branch c : branchlist){ %>
		          <option value =<%=c.getBranchid() %> 
		           <%if(c.getBranchid()==Integer.parseInt(request.getParameter("currentbranchid")==null?"-1":request.getParameter("currentbranchid"))){ %>selected="selected" <%} %> ><%=c.getBranchname()%></option>
		          <%} }%>
			 </select>
			...
			<select id="dispatchdeliveryid" name ="dispatchdeliveryid">
		    	<option value ="-1">请选择配送小件员</option>
			 	...
			 	<option value =<%=c.getUserid() %> 
		           <%if(c.getUserid()==Integer.parseInt(request.getParameter("dispatchdeliveryid")==null?"-1":request.getParameter("dispatchdeliveryid"))){ %>selected="selected" <%} %> >
		           <%=c.getRealname()%>
		           <%if(c.getEmployeestatus()==UserEmployeestatusEnum.LiZhi.getValue()){ %>
		           (离职)
		           <%} %>
				</option>
		          <%}%>
	...
	<tr><td align="left">操作信息
		...
		<select name =operationOrderResultType id ="operationOrderResultType" multiple="multiple" style="width: 320px;">
	...
	<div style="height:120px"></div><%if(orderlist != null && orderlist.size()>0){  %>
	...
	<table width="5000" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
	   <tr class="font_1">
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('cwb');" >入库库房</td>
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" onclick="orderForm('cwb');" >供货商拒收返库备注</td>
	...
	<tr>
	<td class="high">订单总计：<font class="high" size="16px" color="red"><%=count %></font>&nbsp;单  </td>
	</tr>
	<tr>
	<td class="high">代收金额总计：<font class="high" size="16px" color="red"><%=request.getAttribute("sum")==null?"0.00":request.getAttribute("sum") %></font>&nbsp;元 </td>
	</tr>
	<tr>
	<td class="high">代退金额总计：<font class="high" size="16px" color="red"><%=request.getAttribute("paybackfeesum")==null?"0.00":request.getAttribute("paybackfeesum") %></font>&nbsp;元 </td>
	</tr>
	</table>
	<%} %>
	</div><%} %>
