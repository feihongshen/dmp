...
<div class="right_title">
	<div style="background:#FFF">
		...
		<%} %>
		<div class="jg_10"></div>
		<form name="form1" method="post" action="<%=request.getContextPath()%>/smsconfigmodel/createORsave">
		...
		<input type = "hidden" value="<%=channel %>" name="channel"/>
			<table width="100%" border="0" cellspacing="1" cellpadding="5" class="table_5">
				...
				<tr>
					...
					<td align="left"><%if(sms.getIsOpen() == 1){ %>
					<strong>当前使用短信账户【<%=sms.getName()%>】为【<font id="stateId">开启</font>】状态</strong><input name="button" type="button" id="closeOrOpenId" value="关闭" /><input name="button" type="button" id="updateSms" value="修改账户"/>
					<%if(channel==1){ //当当短信渠道首次注册%>
					<br />
					 <input name="button" type="button" id="regist" value="首次激活"  />当当短信渠道首次使用需要激活一次，且只能激活一次，激活多次，可能会被短信渠道提供商屏蔽ip
					 <%} %> 
					<%} else{%>
					<strong>当前使用短信账户【<%=sms.getName()%>】为【<font id="stateId">关闭</font>】状态</strong><input name="button" type="button" id="closeOrOpenId" value="开启" /><input style="display: none" name="button" type="button" id="updateSms" value="修改账户" />
					<%} %>
					</td>
				</tr>
				...
				<tr>
					<td width="120" align="center" bgcolor="#f4f4f4">&nbsp;</td>
					<td align="left">
					<strong>当前没有短信账户，需要从以下设置</strong>
					</td>
				</tr>
				<tr>
					<td align="center" bgcolor="#f4f4f4">帐户：					</td>
		...
		<form id="saveSMS" name="saveSMS" method="post" action="">
			<table width="100%" border="0" cellspacing="1" cellpadding="5" class="table_5">
				<tr>
					...
					<td colspan="4" align="left"><input name="button2" type="button" id="button2" value="查看所有规则" onclick="showAll();"/></td>
				</tr>
				<tr>
					...
					<td width="10%" ><label for="select"></label>
						...
						<td width="10%" id="branchid" >   
						     <select name ="branchids"  id="branchids" multiple="multiple" style="width: 180px;">
							</select>
						</td>
						<td width="10%" >高于：
				...
				<tr>
					<td align="center" bgcolor="#F4F4F4">&nbsp;</td>
					<td colspan="4" align="left"><input name="saveConfig" type="button" class="input_button1" id="saveConfig" value="保存" onclick="saveSms();"></td>
				</tr>
			</table>
		</form>
		...
		预警设置：
		<div class="jg_10"></div>
		<form name="form1" method="post" action="<%=request.getContextPath()%>/smsconfigmodel/createORsave" method="post" id="searchForm">
		<input type="hidden" name ="checksmsnull" value="${checksmsnull}" />
		<input type = "hidden" value="<%=channel %>" name="channel"/>
			<table width="100%" border="0" cellspacing="1" cellpadding="5" class="table_5">
				<tr id="yujing" <%if(sms == null || (sms.getIsOpen()!=1)){ %>style="display: none"<%} %>>
					<td width="10%" align="center" bgcolor="#F4F4F4">打开预警：</td>
					<td colspan="4" align="left"><input type="checkbox" name="monitor" id ="checkboxM" value ="1" <%if(sms != null && sms.getMonitor()==1){ %>checked="checked"<%} %>/></td>
				</tr>
				<tr id="shuliang" <%if(sms == null || (sms.getIsOpen()!=1 ||  sms.getMonitor()!=1)){ %>style="display: none"<%} %>>
					<td width="10%" align="center" bgcolor="#F4F4F4">剩余短信数量即预警：						</td>
					<td colspan="4" align="left"><label for="select">
						<input name="warningcount" type="text" id="textfield5" value="<%=sms==null?0:sms.getWarningcount()%>"/>
					</label></td>
				</tr>
				<tr id="shouji" <%if(sms == null || (sms.getIsOpen()!=1 ||  sms.getMonitor()!=1)){ %>style="display: none"<%} %>>
					<td width="10%" align="center" bgcolor="#F4F4F4">预警发送手机：						</td>
					<td colspan="4" align="left">
					<textarea name="phone" cols="50" rows="5" id="textfield4"><%=sms==null?"":sms.getPhone()%></textarea>
						多个手机用英文逗号隔开</td>
				</tr>
				<tr id="yujingneirong" <%if(sms == null || (sms.getIsOpen()!=1 ||  sms.getMonitor()!=1)){ %>style="display: none"<%} %>>
					<td width="10%" align="center" bgcolor="#F4F4F4"><p>预警内容：</p>
					<p>最多70个字符</p></td>
					<td colspan="4" align="left">
					<textarea name="warningcontent" cols="50" rows="5" id="textfield3"><%=sms==null?"":sms.getWarningcontent()%></textarea></td>
				</tr>
				<tr id="baocunId" <%if(sms == null || (sms.getIsOpen()!=1 ||  sms.getMonitor()!=1)){ %>style="display: none"<%} %>>
					<td width="10%" align="center" bgcolor="#F4F4F4">&nbsp;</td>
					<td colspan="4" align="left"><input type="submit" name="button3" id="button3" value="保存" class="button"  /></td>
				</tr>
			</table>
		</form>
	</div>
