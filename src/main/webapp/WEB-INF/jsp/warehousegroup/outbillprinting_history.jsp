...
if(cwbList.size()>0){
	for(CwbOrder co : cwbList){
		money=money.add(co.getCaramount());
		cwbs += co.getCwb() + ",";
	}
...
function CreateOneFormPage(){
	...
	<%if(iscustomer==1){ %>
		LODOP.PRINT_INIT("<%=printTemplate.getCustomname() %>退供货商出库交接单（补打）");
	<%}else if(operatetype==OutwarehousegroupOperateEnum.FenZhanLingHuo.getValue()){%>
		LODOP.PRINT_INIT("<%=printTemplate.getCustomname() %>领货清单");
	<%}else{ %>
		LODOP.PRINT_INIT("<%=printTemplate.getCustomname() %>出库至<%=branchname %>清单（补打）");
	<%} %>
	LODOP.SET_PRINT_STYLE("FontSize",18);
	LODOP.SET_PRINT_STYLE("Bold",1);
	<%-- <%if(iscustomer==1){ %>
		LODOP.ADD_PRINT_TEXT(50,231,260,39,"<%=printTemplate.getCustomname() %>退供货商出库交接单（补打）");
	<%}else if(operatetype==OutwarehousegroupOperateEnum.FenZhanLingHuo.getValue()){%>
		LODOP.ADD_PRINT_TEXT(50,231,260,39,"<%=printTemplate.getCustomname() %>领货清单");
	<%}else{ %>
		LODOP.ADD_PRINT_TEXT(50,231,260,39,"<%=printTemplate.getCustomname() %>出库至<%=branchname %>清单（补打）");
	<%} %> --%>
