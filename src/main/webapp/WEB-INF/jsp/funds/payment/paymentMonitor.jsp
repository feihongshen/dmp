...
function check(){
	if($(".multiSelectOptions input[name='customerid']:checked").size()==0){
		alert("请选择供货商");
		return false;
	}
	if($("#strtime").val()==""){
...
<div class="right_box">
	...
	<form  id="searchForm" action ="<%=request.getContextPath()%>/funds/paymentfordeliverystate/1" method = "post">
		供货商：<select id="customerid" name="customerid" multiple="multiple" style="width: 200px;">
              ...
              <select name ="dateType" >
		           <option value ="1" <%="1".equals(request.getParameter("dateType"))?"selected":"" %>>反馈时间</option>
		           <option value ="2" <%="2".equals(request.getParameter("dateType"))?"selected":"" %>>推送时间</option>
		    </select><input type ="text" name ="emailStartTime" id="strtime"  value ="<%=StringUtil.nullConvertToEmptyString(request.getAttribute("emailStartTime")) %>">
		...
		配送结果：<select id="deliverystate" name="deliverystate">
					<option value="0">请选择</option>
					<option value="<%=DeliveryStateEnum.PeiSongChengGong.getValue()%>" <%=(DeliveryStateEnum.PeiSongChengGong.getValue()+"").equals(request.getParameter("deliverystate"))?"selected":"" %>><%=DeliveryStateEnum.PeiSongChengGong.getText()%></option>
					<option value="<%=DeliveryStateEnum.ShangMenHuanChengGong.getValue()%>" <%=(DeliveryStateEnum.ShangMenHuanChengGong.getValue()+"").equals(request.getParameter("deliverystate"))?"selected":""%>><%=DeliveryStateEnum.ShangMenHuanChengGong.getText()%></option>
					<option value="<%=DeliveryStateEnum.ShangMenTuiChengGong.getValue()%>" <%=(DeliveryStateEnum.ShangMenTuiChengGong.getValue()+"").equals(request.getParameter("deliverystate"))?"selected":""%>><%=DeliveryStateEnum.ShangMenTuiChengGong.getText()%></option>
					<option value="<%=DeliveryStateEnum.HuoWuDiuShi.getValue()%>" <%=(DeliveryStateEnum.HuoWuDiuShi.getValue()+"").equals(request.getParameter("deliverystate"))?"selected":""%>><%=DeliveryStateEnum.HuoWuDiuShi.getText()%></option>
				</select>
				归班审核状态：
			<select name ="isaudit" id ="isaudit">
		          <option value ="-1" >全部</option>
		           <option value ="0" <%="0".equals(request.getParameter("isaudit"))?"selected":"" %>>未审核</option>
		           <option value ="1" <%="1".equals(request.getParameter("isaudit"))?"selected":"" %>>已审核</option>
		    </select>
	...
	<table width="1600" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
	   <tr class="font_1">
			...
			<td align="center" valign="middle" bgcolor="#eef6ff">实收金额</td>
			...
			<td align="center" valign="middle" bgcolor="#eef6ff">推送时间</td>
			<td align="center" valign="middle" bgcolor="#eef6ff">结算状态</td>
		</tr>
		<%
		...
		%>
		<tr>
			<td align="center" valign="middle" ><%=daohuojsonList.get(j).getString("cwb") %></td>
			<td  align="center" valign="middle" >
				<%for(Branch b : branchList){if(daohuojsonList.get(j).getLong("carwarehouse")==b.getBranchid()){ %>
				<%=b.getBranchname() %>
				<%}} %>
			</td>
			<td  align="center" valign="middle"><%=cumstrMap.get(daohuojsonList.get(j).getLong("customerid")) %></td>              
			<td   align="center" valign="middle"><%=daohuojsonList.get(j).getString("emaildate") %></td>              
			<td   align="center" valign="middle">
				<%for(CwbOrderTypeIdEnum cy : CwbOrderTypeIdEnum.values()){if(daohuojsonList.get(j).getLong("cwbordertypeid")==cy.getValue()){ %>
				<%=cy.getText() %>
				<%}} %>
			</td>
			<td align="center" valign="middle" >
			...
			<td   align="center" valign="middle"><%=daohuojsonList.get(j).getDouble("receivablefee") %></td>
			<td   align="center" valign="middle"><%=daohuojsonList.get(j).getDouble("receivedfee") %></td>
			<td  align="center" valign="middle">
				<%for(PaytypeEnum pe : PaytypeEnum.values()){if(daohuojsonList.get(j).getLong("newpaywayid")==pe.getValue()){ %>
				<%=pe.getText() %>
				<%}} %>
			</td>
			<td  align="center" valign="middle">
				<%for(DeliveryStateEnum de : DeliveryStateEnum.values()){if(daohuojsonList.get(j).getLong("deliverystate")==de.getValue()){ %>
				<%=de.getText() %>
				<%}} %>
			</td>
			<td  align="center" valign="middle"><%=daohuojsonList.get(j).getString("deliverytime") %></td>     
			<td  align="center" valign="middle"><%=daohuojsonList.get(j).getString("auditingtime") %></td>   
			<td  align="center" valign="middle"><%=daohuojsonList.get(j).getString("pushtime") %></td>   
			<td   align="center" valign="middle">
		...
		<tr>
		    <td>合计：</td>
		    <td></td>
			<td colspan="4"><strong><%=count%>[票]</strong></td>              
			<td align="center"><strong>
			<%if(daohuolist.has("receivablefees")&&daohuolist.has("paybackfees")){ %>
			<%if(daohuolist.getDouble("receivablefees")>daohuolist.getDouble("paybackfees")){ %>
	    ...
	    <%-- <tr>
		    <td colspan="13">
		    <%if(customeridList.size()==1 && "1".equals(request.getParameter("deliverystate"))&&"0".equals(request.getParameter("auditState"))){ %>
		    <form action="<%=request.getContextPath()%>/funds/financeauditconfirm" method="POST">
		    	...
		    	<input type="hidden" name="auditCwbOrderType" value="<%=CwbOrderTypeIdEnum.Peisong.getValue()  %>"/>
		    	<input type="hidden" name="type" value="<%=FinanceAuditTypeEnum.AnPeiSongJieGuo.getValue()  %>"/>
		    	<input name="button" type="submit" class="input_button1" id="button" value="审核结算" />
		    </form>
		   
		    <%}else{ %>
		    <font color="red">只有某一个供货商的未审核的配送成功的订单才能在此结算</font>
		    <%} %>
			</td>
	   </tr> --%> 
	</table>
	...
	<form id="excelData" action="<%=request.getContextPath()%>/funds/deliverystatesearchdetail_excel">
		<input type="hidden" name="begin" id="begin" value=""/>
		<input type="hidden" name="dateType1" id="dateType1" value="<%=request.getAttribute("dateType")==null?0:request.getAttribute("dateType") %>"/>
		...
		配送结果：<select id="deliverystate1" name="deliverystate1">
					<option value="0">请选择</option>
					<option value="<%=DeliveryStateEnum.PeiSongChengGong.getValue()%>" <%=(DeliveryStateEnum.PeiSongChengGong.getValue()+"").equals(request.getParameter("deliverystate"))?"selected":"" %>><%=DeliveryStateEnum.PeiSongChengGong.getText()%></option>
					<option value="<%=DeliveryStateEnum.ShangMenHuanChengGong.getValue()%>" <%=(DeliveryStateEnum.ShangMenHuanChengGong.getValue()+"").equals(request.getParameter("deliverystate"))?"selected":""%>><%=DeliveryStateEnum.ShangMenHuanChengGong.getText()%></option>
					<option value="<%=DeliveryStateEnum.ShangMenTuiChengGong.getValue()%>" <%=(DeliveryStateEnum.ShangMenTuiChengGong.getValue()+"").equals(request.getParameter("deliverystate"))?"selected":""%>><%=DeliveryStateEnum.ShangMenTuiChengGong.getText()%></option>
				...
				归班审核状态：
			<select name ="isaudit1" id ="isaudit1">
		          <option value ="-1" >全部</option>
		           <option value ="0" <%="0".equals(request.getParameter("isaudit"))?"selected":"" %>>未审核</option>
		           <option value ="1" <%="1".equals(request.getParameter("isaudit"))?"selected":"" %>>已审核</option>
		    </select>
