...
<script>
	function exportData() {
		var action = "${ctx_path}/overdueexmo/exportdetail";
...
<body style="background: #eef9ff">
	<div class="search_div">
		<form id="searchForm" name="searchForm" class="search_form"
			action="${ctx_path}/overdueexmo/showdetail/1" method="post">
			<input type="hidden" name="optTimeType" value="${cond.optTimeType}" /> <input type="hidden"
				...
				value="${cond.endTime}" /> <input type="hidden" name="orgId" value="${cond.orgId}" /><input
				type="hidden" name="venderId" value="${cond.venderId}" /> <input type="hidden"
				name="showColIndex" value="${cond.showColIndex}" /> <input type="button" id="btnval" value="返回"
				class="input_button2" onclick="javascript:history.back(-1);" /> <input type="button" id="btnval" value="导出"
		...
		<div style="width: 100%">
			<table style="width: 100%" border="0" cellspacing="1" cellpadding="0" class="table_2"
				...
				<tr class="font_1">
					...
					<td width="100" align="center" valign="middle" bgcolor="#eef6ff">供应商</td>
					<td width="100" align="center" valign="middle" bgcolor="#eef6ff">订单创建时间</td>
					<td width="100" align="center" valign="middle" bgcolor="#eef6ff">配送状态</td>
					<td width="100" align="center" valign="middle" bgcolor="#eef6ff">仓库</td>
				...
				<c:forEach items="${result.resultList}" var="tmp">
					<tr>
						...
						<td width="100">${tmp.createDate}</td>
						<td width="100">${tmp.deliverState}</td>
						<td width="100">${tmp.warehouseName}</td>
						<td width="100">${tmp.deliverStationName}</td>
					</tr>
				</c:forEach>
			</table>
		</div>
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_1">
				<tr>
					<td height="38" align="center" valign="middle" bgcolor="#eef6ff"><a
						href="javascript:$('#searchForm').attr('action','${ctx_path}/overdueexmo/showdetail/1');$('#searchForm').submit();">第一页</a>
						<a
						<c:choose>  
                <c:when test="${result.page != 1}">href="javascript:$('#searchForm').attr('action','${ctx_path}/overdueexmo/showdetail/${result.page - 1}');$('#searchForm').submit();"</c:when>  
                <c:otherwise>href="#"</c:otherwise>  
        		</c:choose>>上一页</a>
						<a
						<c:choose>  
                <c:when test="${(result.page != result.pageCount)}">href="javascript:$('#searchForm').attr('action','${ctx_path}/overdueexmo/showdetail/${result.page + 1}');$('#searchForm').submit();"</c:when>  
                <c:otherwise>href="#"</c:otherwise>  
        		</c:choose>>下一页</a>
						<a
						href="javascript:$('#searchForm').attr('action','${ctx_path}/overdueexmo/showdetail/${result.pageCount}');$('#searchForm').submit();">最后一页</a>
						共${result.pageCount}页 共${result.count}条记录 当前第<select id="select"
						onchange="$('#searchForm').attr('action','${ctx_path}/overdueexmo/showdetail/'+$(this).val());$('#searchForm').submit()">
