...
<%for(Customer c : customerList) {%> 
	 if(<%=c.getIfeffectflag()%>==0){
		CustomerKeyAndValue[<%=c.getCustomerid() %>]='<%=c.getCustomername() %><font color=\'red\'>[已停用]</font>';
	 }else{
		 CustomerKeyAndValue[<%=c.getCustomerid() %>]='<%=c.getCustomername() %>';
	 }
<%} %>
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
	   <tr class="font_1">
		...
		<tr>
			...
			[<a href ="javascript:getViewBox(<%=e.getCustomerid()%>);">对应字段详情查看</a>]
			[<a href="javascript:if(confirm('确定要删除?')){del(<%=e.getColumnid()%>);}">删除</a>]
			[<a href="javascript:edit_button(<%=e.getColumnid()%>);">修改</a>]
			<%=((((new Date()).getTime()-sdf.parse(e.getUpdatetime()).getTime())/1000)/3600)>72?"":"["+e.getUpdatetime()+"("+e.getUpdateusername()+")修改"+"]" %>
			</td>
		</tr>
		<%} %>
	</table>
