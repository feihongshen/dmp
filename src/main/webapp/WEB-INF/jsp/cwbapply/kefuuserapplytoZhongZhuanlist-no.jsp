...
function refuseAudit(cwb,applyzhongzhuanbranchid,handleremark){
	if(handleremark.length==0){
		...
		$.ajax({
			...
			data:{
				cwb:cwb,
				applyzhongzhuanbranchid:applyzhongzhuanbranchid,
...
function trueAudit(cwb,applyzhongzhuanbranchid,handleremark){
	if(handleremark.length==0){
		alert("请填写中转审核备注");
	}else if(handleremark.length>50){
		alert("中转审核备注不能超过50字！");
	...
	<div style="overflow-x:scroll; width:100% " id="scroll">
		<table width="1500" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
		   <tr class="font_1">
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" >中转站点</td>
				...
				<td  align="center" valign="middle" bgcolor="#eef6ff" >申请中转站</td>
				<td  align="center" valign="middle" bgcolor="#eef6ff" >中转审核备注</td>
		...
		<% for(CwbOrderView  c : cwbViewList){ %>
			<tr bgcolor="#FF3300">
				...
				<td  align="center" valign="middle"><%=c.getApplyzhongzhuanbranchname() %></td>
				<td  align="center" valign="middle"><%=c.getApplyzhongzhuanremark() %></td>
				...
				<%if((request.getParameter("ishandle")==null?-1:Long.parseLong(request.getParameter("ishandle")))==0){%>
					<td  align="center" valign="middle">
						<select name="zhongzhuanbranchid_<%=c.getCwb() %>" id="zhongzhuanbranchid_<%=c.getCwb() %>">
							<%if(zhongzhuanbranchList!=null&&zhongzhuanbranchList.size()>0)for(Branch b :zhongzhuanbranchList) {%>
							<option value="<%=b.getBranchid() %>" <%if(c.getApplyzhongzhuanbranchid()==b.getBranchid()){ %>selected="selected"<%} %>><%=b.getBranchname() %></option><%} %>
						</select>
					</td>
					...
					<td  align="center" valign="middle">
					<input value="确认审核" type="button" onclick="trueAudit('<%=c.getCwb()%>',$('#zhongzhuanbranchid_<%=c.getCwb() %>').val(),$('#handleremark_<%=c.getCwb() %>').val());"/>
					<input value="客服拒审" type="button" onclick="refuseAudit('<%=c.getCwb()%>',$('#zhongzhuanbranchid_<%=c.getCwb() %>').val(),$('#handleremark_<%=c.getCwb() %>').val());"/>
					</td>
				<%}else if((request.getParameter("ishandle")==null?-1:Long.parseLong(request.getParameter("ishandle")))==2){ %>
					...
					<td  align="center" valign="middle"><input value="<%=c.getApplyzhongzhuanhandleremark() %>" type="text" id="handleremark_<%=c.getCwb() %>" name="handleremark_<%=c.getCwb() %>" maxlength="50" /></td>
					<td  align="center" valign="middle">
					<input value="确认审核" type="button" onclick="trueAudit('<%=c.getCwb()%>',$('#zhongzhuanbranchid_<%=c.getCwb() %>').val(),$('#handleremark_<%=c.getCwb() %>').val());"/>
					</td>
				<%}else{ %>
					<td  align="center" valign="middle">
						<%if(zhongzhuanbranchList!=null&&zhongzhuanbranchList.size()>0)for(Branch b :zhongzhuanbranchList) {if(c.getApplyzhongzhuanbranchid()==b.getBranchid()){ %><%=b.getBranchname() %><%break;}} %>
					</td>
					<td  align="center" valign="middle"><%=c.getApplyzhongzhuanhandleremark() %></td>
					<td  align="center" valign="middle">
					<%if(c.getApplyzhongzhuanishandle()==0){%>未审核<%}else if(c.getApplyzhongzhuanishandle()==1){ %>已审核<%}else{ %>客户拒审<%} %>
					</td>
				<%} %>
			</tr>
		 <% }%>
