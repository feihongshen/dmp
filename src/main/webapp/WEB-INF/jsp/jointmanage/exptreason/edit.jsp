...
ExptReason expt=(ExptReason)request.getAttribute("exptreason");
...
<div id="box_contant">
	...
	<div id="box_in_bg">
		<h1><div id="close_box" onclick="closeBox()"></div>异常码修改</h1>
		<form id="expt_save_Form" name="expt_save_Form"  onSubmit="if(check_exptreason(<%=PosEnum.AliPay.getKey()%>)){submitSaveForm(this);}return false;" action="<%=request.getContextPath()%>/exptreason/save/<%=expt.getExptid() %>" method="post">
		<div id="box_form">
				<ul>
					<li><span>异常码提供方：</span>
						<select name ="support_key" id="support_key" onchange="changethisB2cFlag(this.value,<%=PosEnum.AliPay.getKey()%>)" disabled="disabled">
			               ...
			               <%for(PosEnum p : PosEnum.values()){ %>
			               	<option value ="<%=p.getKey()%>"  <%if(p.getKey()==Integer.parseInt(expt.getSupport_key())){%>selected <%}%>><%=p.getText() %></option>
			               <%} %>
			            </select>
			            
			            <input type="hidden" name="support_key" value="<%=(!"0".equals(expt.getSupport_key())?expt.getSupport_key():("1_"+expt.getCustomerid()))%>"/>
					</li>
					<li><span>供方编码：</span>
 						<input type ="text" id="customercode" name ="customercode" value ="<%=expt.getCustomercode() %>" maxlength="25"  > 
					</li>
					<li id="b2cflag" <%if(Integer.parseInt(expt.getSupport_key())==PosEnum.AliPay.getKey()){%>style="display:" <%}else{%>style="display:none" <%}%>><span>设置针对B2C编码：</span>
					...
					<li><span>备注信息：</span>
					<input  name="expt_remark" id="expt_remark" value="<%=expt.getExpt_remark() %>" size="35"/>
					</li>
				</ul>
		</div>
