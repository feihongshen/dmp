...
<div id="box_contant">
	...
	<div id="box_in_bg">
		<h1><div id="close_box" onclick="closeBox()"></div>退货出库</h1>
		<div id="box_form2">
				<ul>
					<li>
					...
					<li><span>强制出库:</span><input type="checkbox" id="confirmflag" name="confirmflag" value="1"/></li>
					<%} %>
					<li style="display: none"><span>包&nbsp;&nbsp;号：</span><input type="text" class="inputtext_2" name="baleno" id="baleno" maxlength="50"/>
					</li>
					<li><span>扫&nbsp;&nbsp;描：</span><input type="text" class="inputtext_2" id="scancwb" name="scancwb" value="" maxlength="50" onKeyDown='if(event.keyCode==13&&$(this).val().length>0){exportWarehouse("<%=request.getContextPath()%>",$(this).val(),$("#branchid").val(),$("#driverid").val(),$("#truckid").val(),$("#requestbatchno").val(),$("#baleno").val(),$("#ck_switch").val(),$("#confirmflag").attr("checked")=="checked"?1:0);}'/></li>
        ...
        <input type="button" id="finish" name="finish" value="完成扫描" class="button" onclick='backfinishexport("<%=request.getContextPath()%>",$("#branchid").val(),$("#driverid").val(),$("#truckid").val(),$("#requestbatchno").val());'/>
        </div>
	</div>
</div>
