...
$(function(){
	...
	$(function(){
		getChangeOutSum('<%=request.getContextPath()%>',$("#branchid").combobox("getValue"));
...
function addAndRemoval(cwb,tab,isRemoval,branchid){
	...
	//得到当前出库的站点的库存量
	function getChangeOutSum(pname,branchid){
		$.ajax({
			type: "POST",
			url:pname+"/PDA/getChangeOutSum?nextbranchid="+branchid,
			dataType:"json",
...
function exportWarehouse(pname,scancwb,branchid,driverid,truckid,requestbatchno,baleno,ck_switch,confirmflag){
	if(scancwb.length>0){
		if($("#scanbaleTag").attr("class")=="light"){//出库根据包号扫描订单
			...
			$.ajax({
				type: "POST",
				url:pname+"/PDA/cwbchangeoutwarhouse/"+scancwb+"?branchid="+branchid+"&driverid="+driverid+"&truckid="+truckid+"&confirmflag="+confirmflag+"&requestbatchno="+requestbatchno+"&baleno="+baleno,
				dataType:"json",
				success : function(data) {
					$("#scancwb").val("");
...
function chuku(){
	...
	$.ajax({
		type: "POST",
		url:"<%=request.getContextPath()%>/bale/balechangechuku/"+$("#baleno").val()+"?branchid="+$('#branchid').combobox('getValue')+"&driverid="+$("#driverid").val()+"&truckid="+$("#truckid").val(),
		dataType : "json",
		success : function(data) {
			$("#msg").html("");
...
<div class="saomiao_box2">
	...
	<div class="saomiao_tab2">
		<ul>
			...
			<li><a href="<%=request.getContextPath()%>/PDA/cwbchangeoutwarhouseBatch">批量操作</a></li>
		</ul>
	</div>
	...
	<div class="saomiao_topnum2">
		...
		<dl class="yellow">
			<dt>一票多件缺货件数</dt>
			<dd style="cursor:pointer" onclick="tabView('table_quejian');getchukucwbquejiandataList($('#branchid').combobox('getValue'));"  id="lesscwbnum" name="lesscwbnum" >0</dd>
		</dl>
		<input type="button"  id="refresh" value="刷新" onclick="location.href='<%=request.getContextPath() %>/PDA/changeexportwarhouse'"  style="float:left; width:100px; height:65px; cursor:pointer; border:none; background:url(../images/buttonbgimg1.gif) no-repeat; font-size:18px; font-family:'微软雅黑', '黑体'"/>
		<br clear="all"/>
	</div>
		...
		<div class="saomiao_inbox2">
			<div class="saomiao_tab">
				<ul id="bigTag">
					...
					<li><a href="#" id="scanbaleTag" onclick="clearMsg();$(function(){$('#baleno').parent().show();$('#baleno').show();$('#finish').parent().show();$('#finish').show();$('#baleno').val('');$('#baleno').focus();$('#scancwb').val('');$('#scancwb').parent().hide();$('#baleBtn').show();})">合包出库</a></li>
					<%} %>
				</ul>
			</div>
			...
			<div class="saomiao_inwrith2">
				<div class="saomiao_left2">
					...
					<p><span>订单号：</span>
						<input type="text" class="saomiao_inputtxt2" value=""  id="scancwb" name="scancwb"  onKeyDown='if(event.keyCode==13&&$(this).val().length>0){exportWarehouse("<%=request.getContextPath()%>",$(this).val(),$("#branchid").combobox("getValue"),$("#driverid").val(),$("#truckid").val(),$("#requestbatchno").val(),$("#baleno").val(),$("#ck_switch").val(),$("#confirmflag").attr("checked")=="checked"?1:0);}'/>
					</p>
					<p id="baleBtn">
						<span>&nbsp;</span>
						<input type="button" name="fengbao" id="fengbao" value="封包" class="button" onclick="fengbao()"/>
						<input type="button" name="chuku" id="chuku" value="出库" class="button" onclick="chuku()"/>
					</p>
				</div>
		...
		<div>
			<div class="saomiao_tab2">
				...
				<ul id="smallTag">
					...
					<li><a id="table_quejian" href="#" onclick='getchukucwbquejiandataList($("#branchid").combobox("getValue"));getcwbsquejiandataForBranchid($("#branchid").combobox("getValue"))'>一票多件缺件</a></li>
					<li><a href="#">异常单明细</a></li>
				</ul>
			</div>
			<div id="ViewList" class="tabbox">
				<li>
					<input type ="button" id="btnval0" value="导出Excel" class="input_button1" onclick='exportField(1,$("#branchid").combobox("getValue"));'/>
										...
										<table id="weichukuTable" width="100%" border="0" cellspacing="1" cellpadding="2"
											...
											<%} %>
											 <%if(weichukuList!=null&&weichukuList.size()==Page.DETAIL_PAGE_NUMBER){ %>
												<tr align="center"  ><td  colspan="<%if(showCustomerSign){ %>7<%}else{ %>6<%} %>" style="cursor:pointer" onclick="weichuku();" id="weichuku">查看更多</td></tr>
											<%} %>
										</table>
									</div>
								</td>
				...
				<li style="display: none">
					<input type ="button" id="btnval0" value="导出Excel" class="input_button1" onclick='exportField(2,$("#branchid").combobox("getValue"));'/>
					<table width="100%" border="0" cellspacing="10" cellpadding="0">
						<tbody>
							<tr>
								<td width="10%" height="26" align="left" valign="top">
									...
									<div style="height: 160px; overflow-y: scroll">
										<table id="successTable" width="100%" border="0" cellspacing="1" cellpadding="2"	class="table_2">
											...
											<%if(yichukuList!=null&&yichukuList.size()==Page.DETAIL_PAGE_NUMBER){ %>
												<tr  aglin="center"><td colspan="<%if(showCustomerSign){ %>7<%}else{ %>6<%} %>" style="cursor:pointer" onclick="yichuku();" id="yichuku">查看更多</td></tr>
											<%} %>										
										</table>
									</div>
								</td>
				...
				<li style="display: none">
					<input type ="button" id="btnval0" value="导出Excel" class="input_button1" onclick='exportField(3,$("#branchid").combobox("getValue"));'/>
					<table width="100%" border="0" cellspacing="10" cellpadding="0">
						<tbody>
							<tr>
								<td width="10%" height="26" align="left" valign="top">
