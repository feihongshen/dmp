...
<%
List<CwbOrderView> cwborderList = (List<CwbOrderView>)request.getAttribute("cwborderList");
...
int isbranchidflag=request.getAttribute("isbranchidflag")==null?0:1; //标识是否按站点导出
%>
<html>
...
<div class="right_box">
	...
	<form action="<%=request.getContextPath()%>/wharehourecommon/exportExcel" method="post" id="searchForm2">
	...
	<input type="hidden" name="commencode" value="<%=request.getAttribute("commencode")==null?"":request.getAttribute("commencode")%>" >
	<input type="hidden" name="isbranchidflag" value="<%=isbranchidflag%>" >
	<input type="hidden" name="outbranchflag" value="1" >
	
	</form>
	<form id="searchForm1" action ="<%=request.getContextPath()%>/wharehourecommon/showlist_branch" method = "post">
	</form>
	</div>
	   ...
	   <tr class="font_1">
		...
		<% for(CwbOrderView c : cwborderList){ %>
				<tr bgcolor="#FF3300">
					...
					<td  align="center" valign="middle"><%=FlowOrderTypeEnum.getText(c.getFlowordertype())==null?"":FlowOrderTypeEnum.getText(c.getFlowordertype()).getText() %></td>
					<td  align="center" valign="middle">
					<%=customerWarehouseMap.get( Long.parseLong(c.getCustomerwarehouseid()==null?"0":c.getCustomerwarehouseid()))==null?"":customerWarehouseMap.get( Long.parseLong(c.getCustomerwarehouseid()==null?"0":c.getCustomerwarehouseid())).getCustomerwarehouse() %></td>
					<td  align="center" valign="middle"><%=c.getStartbranchname()%></td>
					<td  align="center" valign="middle"><%=c.getNextbranchname() %></td>
					<td  align="center" valign="middle"><%=request.getAttribute("commonname") %></td>
				 </tr>
		 <%} }%>
	</table>
	</div>
	...
	<form  id="searchForm" action="<%=request.getContextPath()%>/wharehourecommon/show/<%=request.getAttribute("commencode")%>/1">
	</form>		
	<div class="jg_10"></div>
