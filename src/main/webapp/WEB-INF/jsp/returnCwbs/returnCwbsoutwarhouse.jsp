...
<%
	List<CwbOrder> weifandanchukulist = (List<CwbOrder>)request.getAttribute("weifandanchukulist");
	List<Customer> customerlist = (List<Customer>)request.getAttribute("customerlist");
	//List<Branch> branchlist = (List<Branch>)request.getAttribute("branchlist");
	List<CwbOrder> yifandanchukulist = (List<CwbOrder>)request.getAttribute("yifandanchukulist");
	String yifandanchukunums=request.getAttribute("yifandanchukunums").toString();
	String weifandanchukunums=request.getAttribute("weifandanchukunums").toString();
	...
	long timetype=Long.parseLong(request.getParameter("timetype")==null?"4":request.getParameter("timetype"));
...
function cwbreturnCwbsbackexport(scancwb,customerid){
	...
	if(scancwb.length>0){
		...
		$.ajax({
			type: "POST",
			url:"<%=request.getContextPath()%>/returnCwbs/cwbreturnCwbsoutwarhouse/"+scancwb+"?customerid="+customerid,
			dataType:"json",
...
function getReturnCwbsbackexportSum(customerandid,timetypewei,starttimewei,endtimewei){
	...
	$.ajax({
		type: "POST",
		url:"<%=request.getContextPath()%>/returnCwbs/getDaiChuKuSum/"+canshu,
		dataType:"json",
		success : function(data) {
...
function search(flag){
	if(flag==1){//待出库
		...
		$("#customerids").val($("#customeridwei").val());	
	...
	if(flag==2){//已出库
		...
		$("#customerids").val($("#customeridyi").val());	
...
<div class="saomiao_box">
	...
	<div class="saomiao_topnum">
		<dl class="blue">
			<dt>今日返单待出库</dt>
			<dd  style="cursor:pointer" onclick="tabView('table_weituihuochuku')" id="chukukucundanshu"><%=weifandanchukunums %></dd>
		</dl>
		
		<dl class="green">
			<dt>今日返单已出库</dt>
			<dd style="cursor:pointer" onclick="tabView('table_yituihuochuku')" id="alloutnum"><%=yifandanchukunums%></dd>
		</dl>
			<input type="button"  id="refresh" value="刷新" onclick="location.href='<%=request.getContextPath() %>/returnCwbs/returnCwbsoutwarhouse'" style="float:left; width:100px; height:65px; cursor:pointer; border:none; background:url(../images/buttonbgimg1.gif) no-repeat; font-size:18px; font-family:'微软雅黑', '黑体'"/>
		<br clear="all">
	</div>
	...
	<div class="saomiao_info2">
		<div class="saomiao_inbox2" style="z-index: 999">
			...
			<div class="saomiao_inwrith2">
				<div class="saomiao_left2">
					<p><span>订单号：</span>
						<input type="text" class="saomiao_inputtxt" value="" id="scancwb" name="scancwb"  onKeyDown='if(event.keyCode==13&&$(this).val().length>0){cwbreturnCwbsbackexport($(this).val(),$("#customerid").val());}'/>
					</p>
				</div>
 	...
 	<div>
			<div class="saomiao_tab2">
				...
				<ul>
					<li ><a id="table_weituihuochuku" href="#" >待出库明细</a></li>
								...
								<td width="10%" height="26" align="left" valign="top">
									<table width="100%" border="0" cellspacing="0" cellpadding="2"
										...
										<tr>
											...
											<td width="140" align="center" bgcolor="#f1f1f1">入库时间</td> 
									...
									<div style="height: 160px; overflow-y: scroll">
										<table id="weituihuochukuTable" width="100%" border="0" cellspacing="1" cellpadding="2"
											class="table_2">
											<%for(CwbOrder co : weifandanchukulist){ %>
						...
						<tbody>
							<tr>
								<td width="10%" height="26" align="left" valign="top">
									...
									<div style="height: 160px; overflow-y: scroll">
										<table id="successTable" width="100%" border="0" cellspacing="1" cellpadding="2" class="table_2">
											<%for(CwbOrder co : yifandanchukulist){ %>
											<tr id="TR<%=co.getCwb() %>" cwb="<%=co.getCwb() %>" customerid="<%=co.getCustomerid() %>" nextbranchid="<%=co.getNextbranchid() %>" >
												<td width="120" align="center"><%=co.getCwb() %></td>
												<td width="100" align="center"><%for(Customer c:customerlist){if(c.getCustomerid()==co.getCustomerid()){out.print(c.getCustomername());break;}} %></td>
		...
		<form action="<%=request.getContextPath()%>/PDA/exportExcle" method="post" id="searchForm2">
 		...
 		<form action="<%=request.getContextPath() %>/returnCwbs/returnCwbsoutwarhouse" method="post" id="searchForm">
			<input type="hidden" name="customerid" id="customerids" value="" />
