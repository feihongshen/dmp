...
String flag=(String)request.getAttribute("dd")==null?"":(String)request.getAttribute("dd");
String iSXinjiang=(String)request.getAttribute("xjdcb")==null?"":(String)request.getAttribute("xjdcb");
...
<div class="right_box">
	<div class="inputselect_box">
		<form action="<%=request.getContextPath() %>/cwbLablePrint/cwblableprint" method="post" id="searchForm">
			...
			供货商：<select id ="customerid" name ="customerid" onchange="customeridchange($(this).val());"> 
				...
				<%if(clist!=null&&clist.size()>0&&flag.length()>0&&flag.equals("dd")){ %>
			      <select id="moudle1" >
			      	<option value="DD_LOGO.jpg">10*12</option>
			      	<option value="DINGDONG_LOGO.jpg">10*16</option>
			      </select>
		      <%} %>
		      <%if(clist!=null&&clist.size()>0&&iSXinjiang.length()>0&&iSXinjiang.equals("xjdcb")){ %>
			      打印模板<select id="moudle1" >
			      	<option value="xjdcb_LOGO.jpg">新疆大晨报</option>
			      </select>
		      <%} %>
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table2">
		   <tbody>
			   	...
			   	<form action="<%=request.getContextPath() %>/cwbLablePrint/cwblableprint_xhm" method="post" id="selectforprintForm" >
			   	<input name="printtemplet" value="${printtemplet }" type="hidden"/>
			   	<input name="moudle" id="moudle" value="" type="hidden"/>
			   	<%if(clist!=null&&clist.size()>0){for(CwbOrder c : clist){ %>
				   	<tr style="background-color: rgb(249, 252, 253); ">
						...
						<td align="center" valign="middle"><%if(branchList!=null&&branchList.size()>0)for(Branch b : branchList){if(c.getDeliverybranchid()==b.getBranchid()){%><%=b.getBranchname() %><%}} %></td>
						...
						<td align="center" valign="middle"><%=c.getConsigneemobile().length()==0?c.getConsigneephone():c.getConsigneemobile() %></td>
