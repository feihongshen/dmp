...
function CreateOneFormPage(cwbs){
	...
	for (i = 0; i < <%=cwbList.size()%>; i++) {
		...
		LODOP.ADD_PRINT_BARCODE(145,210,150,70,"128Auto", cwb);
...
*{font-size:12px; line-height:20px; margin:0; padding:0; font-family:宋体}
					...
					<td>
						<table width="100%" border="1" cellspacing="0" cellpadding="5" class="table1">
							<tr>
								...
								<div style="float:right; width:200px; line-height:20px">&nbsp;<%=co.getEmaildate().substring(0, 10) %>&nbsp;中兴云购网
								&nbsp;<%if(branchlist!=null&&branchlist.size()>0)for(Branch b : branchlist){if(co.getDeliverybranchid()==b.getBranchid()){ %><%=b.getBranchname() %><%}} %></div>
								</td>
							</tr>
							<tr>
								<td width="49" align="center">信息</td>
								<td colspan="2" align="center" style="padding: 1px 0;height:80"><p><%=co.getCwbprovince() %>……<%=co.getCwbcity() %>……<%=co.getCwbcounty() %>&nbsp;&nbsp;邮编：<%=co.getConsigneepostcode() %></p>
									<p style="line-height: 12px;"><%=co.getConsigneeaddress() %>客户：<%=co.getConsigneename() %>&nbsp;&nbsp;电话：<%=co.getConsigneemobile().length()==0?co.getConsigneephone():co.getConsigneemobile() %></p></td>
							</tr>
							<tr>
								<td rowspan="2" align="center">金额</td>
								<td width="205" align="center">货品价值：<%=co.getCaramount() %>元</td>
								<td width="238" rowspan="2" align="center" id="printcwb<%=co.getCwb() %>" name="printcwb<%=co.getCwb() %>">&nbsp;</td>
							</tr>
							<tr>
								<td align="center">代收款：<%=co.getReceivablefee() %>元（现金）</td>
							</tr>
						</table>
						<table width="100%" border="1" cellspacing="0" cellpadding="5" style="margin-top:-2px">
							<tr>
								...
								<td rowspan="2" align="center"><p>客户签名：</p>
									<p>时间：&nbsp;&nbsp;年&nbsp;&nbsp;月&nbsp;&nbsp;日</p></td>
							</tr>
							<tr>
								<td align="center">件数：<%=co.getSendcarnum() %>（件）</td>
							</tr>
						</table>
						...
						<table width="100%" border="1" cellspacing="0" cellpadding="5" class="table2">
							<tr>
								...
								<td><p>配送单号：<%=co.getCwb() %></p></td>
							</tr>
							<tr>
								<td style="line-height: 12px;">客户：<%=co.getConsigneename() %>&nbsp;电话：<%=co.getConsigneemobile().length()==0?co.getConsigneephone():co.getConsigneemobile() %></td>
								<td>应收金额：<%=co.getReceivablefee() %>元</td>
							</tr>
							<tr>
								<td><p>网址：www.zxcloud.com </p>
									<p style="line-height: 12px;">热线电话：4000-818-222</p></td>
								<td rowspan="2" style="line-height: 12px;">保留此单包裹单底代表您已经签收并认可我们的服务，如有任何疑问可联系客服咨询。</td>
							</tr>
							<tr>
								<td>中兴云购网：www.zxcloud.com</td>
							</tr>
						</table>
