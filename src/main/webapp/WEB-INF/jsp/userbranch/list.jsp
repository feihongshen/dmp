...
<div class="right_box">
	...
	<tr class="font_1">
		...
		<%if(userbranchList != null && userbranchList.size()>0 && mapList != null && !mapList.isEmpty()){ %>
		 <% for(UserBranch ub : userbranchList){ String username=""; %>
		 <%for(User u : userlist){if(ub.getUserid()==u.getUserid()){ %>
			<%username=u.getRealname() ;break;}
			} %>
		...
		<tr>
			...
			<%for(UserBranch csc : mapList.get(ub.getUserid()+"List")){ %>
			  <%for(Branch b : branchlist){if(b.getBranchid()==csc.getBranchid()){ %>
				<%=b.getBranchname() %><%if(mapList.get(ub.getUserid()+"List").size()>1){ %> | <%} %>
			  <%break;}
				} %>
			<%} %>
			</td>
			<td width="10%" align="center" valign="middle" >
			[<a href="javascript:if(confirm('确定要删除?')){del('<%=ub.getUserid() %>');}">删除</a>]
			[<a href="<%=request.getContextPath()%>/userBranchControl/edit/<%=ub.getUserid() %>;">修改</a>]
			</td>
		</tr>
		<%} }}%>
	</table>
...
<!-- 创建用户区域权限的ajax地址 -->
<input type="hidden" id="add" value="<%=request.getContextPath()%>/userBranchControl/add" />
<!-- 修改用户区域权限的ajax地址 -->
<input type="hidden" id="edit" value="<%=request.getContextPath()%>/userBranchControl/edit/" />
<!-- 删除用户区域权限的ajax地址 -->
<input type="hidden" id="del" value="<%=request.getContextPath()%>/userBranchControl/del/" />
</body>
</html>