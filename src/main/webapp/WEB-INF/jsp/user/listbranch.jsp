...
<div class="right_box">
	...
	<form action="<%=request.getContextPath() %>/user/listbranch/<%=request.getAttribute("page")==null?"1":request.getAttribute("page") %>" method="post" id="searchForm" method="post" >
		...
		<input type="submit" onclick="$('#searchForm').attr('action','<%=request.getContextPath() %>/user/listbranch/1');return true;" id="find" value="查询" class="input_button2" />
		<input type="button"  onclick="location.href='<%=request.getContextPath() %>/user/listbranch/1'" value="返回" class="input_button2" />
	</form>
	</div>
	...
	<tr class="font_1">
			...
			<td width="15%" align="center" valign="middle" bgcolor="#eef6ff">登录名</td>
		...
		<tr>
			...
			<td width="15%" align="center" valign="middle" ><%=branch.getBranchname() %></td>
			<td width="10%" align="center" valign="middle"><%if(u.getRoleid()==4){ %>站长<%}else{ %>小件员<%} %></td>
			<td width="10%" align="center" valign="middle" ><%=u.getRealname() %></td>
			<td width="15%" align="center" valign="middle" ><%=u.getUsername() %></td>
	...
	<tr>
		<td height="38" align="center" valign="middle" bgcolor="#eef6ff">
			<a href="javascript:$('#searchForm').attr('action','<%=request.getContextPath() %>/user/listbranch/1');$('#searchForm').submit();" >第一页</a>　
			<a href="javascript:$('#searchForm').attr('action','<%=request.getContextPath() %>/user/listbranch/<%=page_obj.getPrevious()<1?1:page_obj.getPrevious() %>');$('#searchForm').submit();">上一页</a>　
			<a href="javascript:$('#searchForm').attr('action','<%=request.getContextPath() %>/user/listbranch/<%=page_obj.getNext()<1?1:page_obj.getNext() %>');$('#searchForm').submit();" >下一页</a>　
			<a href="javascript:$('#searchForm').attr('action','<%=request.getContextPath() %>/user/listbranch/<%=page_obj.getMaxpage()<1?1:page_obj.getMaxpage() %>');$('#searchForm').submit();" >最后一页</a>
			　共<%=page_obj.getMaxpage() %>页　共<%=page_obj.getTotal() %>条记录 　当前第<select
					id="selectPg"
					onchange="$('#searchForm').attr('action',<%=request.getContextPath() %>/user/listbranch/$(this).val());$('#searchForm').submit()">
...
<!-- 创建订单类型的ajax地址 -->
<input type="hidden" id="add" value="<%=request.getContextPath()%>/user/addBranch" />
<!-- 修改订单类型的ajax地址 -->
<input type="hidden" id="edit" value="<%=request.getContextPath()%>/user/editBranch/" />
<!-- 删除订单类型的ajax地址 -->
<input type="hidden" id="del" value="<%=request.getContextPath()%>/user/delBranch/" />
</body>
</html>