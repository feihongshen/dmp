...
function CreatePrintPage(cwbs) {
	...
	for(var i=0;i<cwbs.toString().split(",").length;i++){
		...
		if(i>0){h=h+560;}
		LODOP.ADD_PRINT_BARCODE(h+130,450,200,60,"128Auto", cwb);
...
<%} %>
		<form id="form1">
			<%for(CwbOrder co : clist){ %>
					<table id="printTable<%=clist.indexOf(co) %>"  height="531" width="100%" border="1" cellspacing="0" cellpadding="0" class="table_dd" >
					<tr>
				   ...
				   if(co.getRemark5() != null && co.getRemark5().indexOf("partDesc")>-1){
					   ...
					   if(jsonArr!= null && jsonArr.size() >0){
						   for(int i=0;i<jsonArr.size();i++){
							   ...
							   partDescname += i+1+"."+JSONObject.fromObject(jsonArr.get(i)).getString("partDesc")+" ";
							   packingListname += i+1+"."+JSONObject.fromObject(jsonArr.get(i)).getString("packingList")+" ";
						   }
					   }
				   }
				   %>
					  <td rowspan="6" align="left" valign="middle">货物信息</td>
					  <td colspan="2" height="30px" align="left" valign="top">货物名称：<%=StringUtil.nullConvertToEmptyString(partDescname)  %>，
					  包装清单：<%=StringUtil.nullConvertToEmptyString(packingListname) %></td>
				  </tr>
					<tr>
					...
					<tr>
					  <td colspan="2" align="left" valign="top"><p>货品描述：<%=StringUtil.nullConvertToEmptyString(partDescname) %></p></td>
				  </tr>
				  <tr>
					  <td colspan="2" align="left" valign="top"><p>备注：<%=StringUtil.nullConvertToEmptyString(co.getRemark3())%></p></td>
				  </tr>
					<tr>
