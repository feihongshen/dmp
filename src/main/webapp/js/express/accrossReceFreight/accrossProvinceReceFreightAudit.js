//问题记录：此处的文档加载后执行的函数不生效   ---------原因引入的时间插件datepickerjs冲突现已删除
$(function(){
	//获得省份
	getProvince();
	//为查询条件赋值
...
function getProvince(){
	$.ajax({
		...
		url : App.ctx + "/acrossProvinceReceFreightAudit/getProvinces",
		datatype : "json",
		success : function(result) {
			...
			$("#receivableProvinceId").append(selectorArray.join(""));
		}
	});
}
...
function valuTion(){
	...
	$("#receivableProvinceId").val($("#receivableProvinceIdValue").val());
...
function initColor(td_$){
	...
	var id = 'acrossProvinceAuditTable';
	var ids = ""; 
	var count = 0;
...
function edit(id){
	...
	$.ajax({
		type : "post",
		async : false, // 设为false就是同步请求
		url :  App.ctx+"/acrossProvinceReceFreightAudit/edit",
		data : params,
		datatype : "json",
		success : function(result) {
			//为编辑弹出框赋值			
			...
			$("#receivableProvinceNameEdit").val(result.receivableProvinceName);
			$("#payableProvinceNameEdit").val(result.payableProvinceName);
	...
	var table = $("#acrossProvinceAuditEdit");
	//账单下的订单明细
	$.ajax({
		type : "post",
		async : false, // 设为false就是同步请求
		url :  App.ctx+"/acrossProvinceReceFreightAudit/orderDetailsPageForBill",
		data : params,
		datatype : "json",
		success : function(result) {
...
function audit(){
	...
	$.ajax({
		type : "post",
		async : false, // 设为false就是同步请求
		url :  App.ctx+"/acrossProvinceReceFreightAudit/audit",
		data : params,
		datatype : "json",
		success : function(result) {
			if(result==1){
...
function cancalAudit(){
	...
	$.ajax({
		type : "post",
		async : false, // 设为false就是同步请求
		url :  App.ctx+"/acrossProvinceReceFreightAudit/cancelAudit",
		data : params,
		datatype : "json",
		success : function(result) {
			if(result==1){
...
//绑定核销按钮的点击事件
//$("#checkAuditButton").click(function(){
function checkAudit(){
	ids = getIds();
	var state = $("#billStateEdit").val();
	var params = {};
	...
	$.ajax({
		type : "post",
		async : false, // 设为false就是同步请求
		url :  App.ctx+"/acrossProvinceReceFreightAudit/checkAudit",
		data : params,
		datatype : "json",
		success : function(result) {
			if(result==1){
...
function exportTable(){
	...
	params.pageNo = 1;
		...
		url :  App.ctx+"/acrossProvinceFreightAudit/exportBill",
		data : params,
		datatype : "json",
		success : function(result) {
		}
