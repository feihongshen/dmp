 ...
 */
var cwbDetail = {};
function printIntowarehouseNew(scancwb){
	if(scancwb.length>0){
		$.ajax({
			type: "POST",
 ...
 * @param scancwb
 */
function CreateOneFormPage1(scancwb,printHtml){

	LODOPS=getLodop(App.ctx,document.getElementById('LODOP'),document.getElementById('LODOP_EM'));  
	LODOPS.NEWPAGE();
	LODOPS.PRINT_INIT("订单打印");
	LODOPS.SET_PRINT_STYLE("Bold",1);
	LODOPS.ADD_PRINT_TABLE(10,10,400,250,printHtml);
 ...
 * @param scancwb
 */
function prn1_print1(scancwb,printHtml) {		
	CreateOneFormPage1(scancwb,printHtml);
	LODOPS.PRINT();
};