SELECT '1.清空人员管理中打印收件人电话手机设置' AS '脚本注释',IF((
select count(1) from express_set_user where showphoneflag != 0 or showmobileflag != 0
)= 0 ,'success','failed') AS '执行结果'
UNION ALL
SELECT '2.在导出字段可选列表中删除收件人电话手机' AS '脚本注释',IF((
select count(1) from express_ops_setexportfield where id in (8, 9)
)= 0 ,'success','failed') AS '执行结果'
UNION ALL
SELECT '3.在导出字段已选列表中删除收件人电话手机' AS '脚本注释',IF((
