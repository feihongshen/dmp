...
<%
	List<CwbOrder> smtlist = (List<CwbOrder>) request.getAttribute("smtlist");
	List<Customer> customerlist = (List<Customer>) request.getAttribute("customerlist");
	String cwbs = "";

	if (smtlist!=null&&smtlist.size() > 0) {
		for (CwbOrder smtcd : smtlist) {
			cwbs += smtcd.getCwb() + ",";
		}
	}
	if(cwbs.length()>0){
...
function CreatePrintPage(cwbs) {
	...
	for(var i=0;i<cwbs.toString().split(",").length;i++){
		...
		LODOP.ADD_PRINT_BARCODE(document.getElementById("printcwb"+cwb).offsetTop+document.getElementById("printTable"+num).offsetTop+h+78,
				document.getElementById("printcwb"+cwb).offsetLeft+document.getElementById("TD"+cwb).offsetLeft+32,150,30,"128Auto", cwb);
