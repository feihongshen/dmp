...
<body>
	...
	<div  >
		...
		<div id="box_in_bg">
			<h1>修改</h1>
	<form action="<%=request.getContextPath() %>/notify/save"  method="post"  enctype="multipart/form-data"  id="form1">
			<input type="hidden" name="id" value="<%=nf.getId()%>" />
			<table width="100%" border="0" cellspacing="1" cellpadding="5" class="table_2">
				<tr class="font_1">
					...
					<input type="text" name="title" id="title" value="<%=nf.getTitle() %>" style="height:30px;width:400px"/></td>
				</tr>
				<tr>
					<td align="left"><input type="radio" name="type" <%if(nf.getType()==1){ %>checked="true"<%} %>  value="1"  />
						公告
					<label for="radio">
						<input type="radio"  name="type"  <%if(nf.getType()==2){ %>checked="true"<%} %>  value="2"/>
						通知
					</label>
					</td>
				</tr>
				<tr>
					...
					<textarea id="editor" type="text/plain" name="content" style="width:100%;height:350px;"><%=nf.getContent() %></textarea>
					</td>
				</tr>
			</table>
		<div align="center">
