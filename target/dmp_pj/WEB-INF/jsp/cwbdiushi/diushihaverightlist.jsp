...
function todetial(id){
	$.ajax({
		type: "POST",
		url:"<%=request.getContextPath()%>/cwbdiushi/diushishow/"+id,
		dataType:"html",
	   ...
	   <tr class="font_1">
		...
		<% for(CwbDiuShiView  cds : cwbDiuShiViewList){ %>
				<tr bgcolor="#FF3300">
					...
					<%if(cds.getIshandle()==CwbDiuShiIsHandleEnum.WeiChuLi.getValue()){ %>
						<a href="javascript:;" onclick="todetial(<%=cds.getDiushiid()%>)"><%=CwbDiuShiIsHandleEnum.WeiChuLi.getText() %></a>
					<%}else{ %>
						<%=CwbDiuShiIsHandleEnum.YiChuLi.getText() %>
					<%} %>
					</td>
				 </tr>
		 <% }%>
