...
<%
List<CwbOrder> cwborderList = request.getAttribute("cwbList")==null?new ArrayList<CwbOrder>():(List<CwbOrder>)request.getAttribute("cwbList");
...
<div class="right_box">
	...
	<form id="searchForm2" action ="<%=request.getContextPath()%>/ExceptionMonitor/exportException" method = "post" > 
			...
			<tr>
				...
				<input type ="button" id="export" value="导出" class="input_button2" onclick="exportException()"/>
				
			</td>
			</tr>
			</table>
	</form>
	<form id="searchForm1" action ="<%=request.getContextPath()%>/ExceptionMonitor/operationTimeOut" method = "post">
    ...
    <%} %>
		...
		<form  id="searchForm" 
		action="<%=request.getContextPath()%>/ExceptionMonitor/showTimeOut/<%=request.getAttribute("outhour")==null?"0":request.getAttribute("outhour")%>/<%=request.getAttribute("branchid")==null?"":request.getAttribute("branchid")%>/<%=request.getAttribute("flowordertype")==null?0:request.getAttribute("flowordertype")%>/<%=request.getAttribute("deliverystate")==null?0:request.getAttribute("deliverystate")%>/<%=customerid%>/
		<%=begindate.equals("")?"-":begindate%>/<%=enddate.equals("")?"-":enddate%>/1">
		</form>	
	<div class="jg_10"></div>
