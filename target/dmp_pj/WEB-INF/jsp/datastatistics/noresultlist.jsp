...
<%
  ...
  List kufangidstr =(List) request.getAttribute("kufangidstr");
  List branchidstr =(List) request.getAttribute("branchidstr");
...
<script type="text/javascript">
	$(document).ready(function() {
		 $("#kufangid").multiSelect({ oneOrMoreSelected: '*',noneSelected:'请选择发货库房' });
		  $("#branchid").multiSelect({ oneOrMoreSelected: '*',noneSelected:'请选择到货站点' });
	   //获取下拉框的值
	   $("#find").click(function(){
	         if(check()){
	            $("#isshow").val(1);
...
<div class="right_box">
	...
	<tr>
		<td align="left">
			...
			<select name="ismohu" id="ismohu" >
			 ...
			 <input name="branchname" id="branchname" onKeyDown="if(event.keyCode==13&&$(this).val().length>0){moHuOrJingQueSlect($('#ismohu').val(),'<%=request.getContextPath()%>','kufangids',$(this).val());}"/>
			发货库房：
		    <select name ="kufangids" id ="kufangid" multiple="multiple" style="width: 300px;">
		          <%if(kufanglist!=null && kufanglist.size()>0) {%>
		          <%for(Branch c : kufanglist){ %>
			 			<option value =<%=c.getBranchid() %> 
		         <%if(!kufangidstr.isEmpty()) 
						            {for(int i=0;i<kufangidstr.size();i++){
						            	if(c.getBranchid()== new Long(kufangidstr.get(i).toString())){
						            		%>selected="selected"<%
						            	 break;
						            	}
						            }
					     }%>><%=c.getBranchname()%></option>
		          <%} }%>
			</select>[<a href="javascript:multiSelectAll('kufangids',1,'请选择');">全选</a>][<a href="javascript:multiSelectAll('kufangids',0,'请选择');">取消全选</a>]
			<br />
			到货站点名称：
			<select name="ismohu" id="ismohu" >
					<option value ="1"<%if(1==(request.getParameter("ismohu")==null?1:Long.parseLong(request.getParameter("ismohu")))){%>selected="selected"<%}%>>模糊匹配</option>
			 ...
			 <input name="branchname" id="branchname" onKeyDown="if(event.keyCode==13&&$(this).val().length>0){moHuOrJingQueSlect($('#ismohu').val(),'<%=request.getContextPath()%>','branchids',$(this).val());}"/>
			到货站点：
			<select name ="branchids" id ="branchid"  multiple="multiple" style="width: 300px;">
		          <%if(branchlist!=null && branchlist.size()>0){ %>
		          <%for(Branch c : branchlist){ %>
			 			<option value =<%=c.getBranchid() %> 
		         <%if(!branchidstr.isEmpty()) 
						            {for(int i=0;i<branchidstr.size();i++){
						            	if(c.getBranchid()== new Long(branchidstr.get(i).toString())){
						            		%>selected="selected"<%
						            	 break;
						            	}
						            }
		          ...
		          <%} }%>
			 </select>[<a href="javascript:multiSelectAll('branchids',1,'请选择');">全选</a>][<a href="javascript:multiSelectAll('branchids',0,'请选择');">取消全选</a>]
	...
	<form action="<%=request.getContextPath()%>/datastatistics/exportNoresult" method="post" id="searchForm2">
		<input type="hidden" name="begin" id="begin" value="0"/>
		<input type="hidden" name="exportmould" id="exportmould2" />
		<input type="hidden" name="begindate" id="begindate1" value="<%=starttime%>"/>
		<input type="hidden" name="enddate" id="enddate1" value="<%=endtime%>"/>
		<div style="display: none;">
		 <select name ="kufangids" id ="kufangid1" multiple="multiple" style="width: 100px;">
		          <%if(kufanglist!=null && kufanglist.size()>0) {%>
		          <%for(Branch c : kufanglist){ %>
			 			<option value =<%=c.getBranchid() %> 
		         <%if(!kufangidstr.isEmpty()) 
			...
			<select name ="branchids" id ="branchid1"  multiple="multiple" style="width: 100px;">
		          <%if(branchlist!=null && branchlist.size()>0){ %>
		          <%for(Branch c : branchlist){ %>
			 			<option value =<%=c.getBranchid() %> 
		         <%if(!branchidstr.isEmpty()) 
	...
	<table width="1500" border="0" cellspacing="1" cellpadding="0" class="table_2" id="gd_table">
	   <tr class="font_1">
		...
		<tr bgcolor="#FF3300">
			<td class="high" align="left" valign="middle" colspan="6">&nbsp;&nbsp;&nbsp;合计：<font color="red"><%=count %></font>&nbsp;单  <font color="red"><%=request.getAttribute("sum")==null?"0.00":request.getAttribute("sum") %></font>&nbsp;元 </td>
		</tr>
		<%} %>
	</table>
