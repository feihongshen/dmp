...
<div id="box_contant">
	...
	<div id="box_in_bg">
		...
		<form id="expt_save_Form" name="expt_save_Form"  onSubmit="if(check_exptreason(<%=PosEnum.AliPay.getKey()%>)){submitSaveForm(this);}return false;" action="<%=request.getContextPath()%>/Commreason/create/" method="post">
		<div id="box_form">
				<ul>
					<li><span>承运商：</span>
					...
					<option value="-1">请选择</option>
						<%if(customerlist!=null&&customerlist.size()>0){
							for(Common cu:customerlist){%>
								<option value="<%=cu.getId()%>"><%=cu.getCommonname() %></option>
							<%}
						} %>
