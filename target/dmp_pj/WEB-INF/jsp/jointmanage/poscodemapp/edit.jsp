...
PoscodeMapp expt=(PoscodeMapp)request.getAttribute("poscodemapp");
...
<div id="box_contant">
	...
	<div id="box_in_bg">
		<h1><div id="close_box" onclick="closeBox()"></div>POS商户编码修改</h1>
		<form id="expt_save_Form" name="expt_save_Form"  onSubmit="submitSaveForm(this);return false;" action="<%=request.getContextPath()%>/poscodemapp/save/<%=expt.getPoscodeid()%>" method="post">
		<div id="box_form">
				<ul>
					<li><span>POS支付方式：</span>
						<select name ="posenum" id="posenum" disabled>
			               <option value ="-1">请选择</option>
			               <%for(PosEnum p : PosEnum.values()){ %>
			               
			               	<option value ="<%=p.getKey()%>" <%if(expt.getPosenum()==p.getKey()){%>selected<%}%>><%=p.getText() %></option>
			               <%} %>
			            </select>
