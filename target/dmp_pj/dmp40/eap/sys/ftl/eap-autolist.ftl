...
$(function(){
	...
	<#if config_querymode == "group">
		creatQueryTableItems();
	</#if>
});

<#-- 创建表格查询item-->
function creatQueryTableItems() {
	var parentObj = jQuery("div[id='${config_id}_searchColums']"); 
	jQuery("table[id='${config_id}query_table']", parentObj).find("td").each(function() {
		var _thisTd = jQuery(this);
		parentObj.find("span").each(function() {
			if (jQuery(this).attr("code") === _thisTd.attr("code")) {
				jQuery(this).appendTo(_thisTd);
			}
		});		
	});
}
...
function createDataGrid${config_id}(){
	...
	onLoadSuccess:function(data){
		...
		for (var i = 0; i < listJs.length; i++) {
			if(listJs[i]) EapTools.loadJs(listJs[i], _viewer);
		}
	},
	...
	<#list config_nolist as x>
		$("#${config_id}Listtb").find("#${x}").remove();
	</#list>
	}
	//列表刷新
...
<div name="searchColums" style="margin-top:15px;" id="${config_id}_searchColums">
<#if config_querymode == "group">
	<table class="table table-striped table-bordered table-hover table-condensed" id="${config_id}query_table">
		<#list trList as tr>
			<tr>
				<#list tr as td>
					${td}
	...
	<#list config_queryList  as x>
		<#if x['field_isQuery']=="Y">
			<span style="display:-moz-inline-box;display:inline-block;" name="query_item" code="${x['field_id']}">
				<span style="display:-moz-inline-box;display:inline-block;margin:0px 0px -6px 0px;text-align:right;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap;" title="${x['field_title']}">${x['field_title']}：</span>
		</#if>
		<#if x['field_queryMode']=="group">
			<#if x['field_isQuery']=="Y">
			<input type="text" name="${x['field_id']}_begin"  style="height:15px;width: 94px;margin-bottom:0px;"  <#if x['field_type']=="Date">class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"</#if> value="${x['field_value_begin']}" />
			<span style="display:-moz-inline-box;display:inline-block;width: 8px;text-align:right;">~</span>
			<input type="text" name="${x['field_id']}_end"  style="height:15px;width: 94px;margin-bottom:0px;" <#if x['field_type']=="Date">class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"</#if> value="${x['field_value_end']}"/>
			<#else>
		...
		<#if x['field_queryMode']=="single">
			<#if x['field_isQuery']=="Y">
				<#if  (x['field_dictlist']?size >0)>
					<select name = "${x['field_id']}" WIDTH="100" style="width: 104px;margin-bottom:0px;">
				...
				<#if  (x['field_dictlist']?size <= 0)>
					...
					<input type="text" name="${x['field_id']}"  style="height:15px;width: 100px;margin-bottom:0px;" <#if x['field_type']=="Date">class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"</#if>  value="${x['field_value']?if_exists?default('')}" />
					<#else>
	...
	<div class="datagrid-toolbar" style="height:30px; margin:10px 0px 10px 0px; border-bottom:0px;">
	<span style="float:left;">
		<div id="add" class="btn btn-default" plain="true" style="margin-right:5px;" onclick="${config_id}add()">
			<i class="icon-plus"></i>录入
		</div>
		<div id="update" class="btn btn-default" plain="true" style="margin-right:5px;" onclick="${config_id}update()">
			<i class="icon-edit"></i>编辑
		</div>
		<div id="delete" class="btn btn-default" plain="true" style="margin-right:5px;" onclick="${config_id}delBatch()">
			<i class="icon-remove"></i>批量删除
		</div>
		<div id="detail" class="btn btn-default" plain="true" style="margin-right:5px;" onclick="${config_id}view()">
			<i class="icon-search"></i>查看
		</div>
		<div id="import" class="btn btn-default" plain="true" style="margin-right:5px;" onclick="add('${config_name}Excel数据导入','excelTempletController.do?goImplXls&tableName=${config_id}','${config_id}List')">
			<i class="icon-arrow-up"></i>Excel数据导入
		</div>
		<div id="excel" class="btn btn-default" plain="true" style="margin-right:5px;">
			<i class="icon-download-alt"></i><a href="excelTempletController.do?exportXls&tableName=${config_id}">Excel模板下载</a>
		</div>
	<#list config_buttons as x>
		<#if x['buttonStyle'] == 'button' && x['buttonStatus']=='1'>
			<div id="${x['buttonCode']}" class="btn btn-default" plain="true"
			<#if x['optType'] == 'action'>
				onclick="doBusButton('cgFormBuildController.do?doButton&formId=${x['formId']}&buttonCode=${x['buttonCode']}&tableName=${config_id}','${x['buttonName']}','${config_id}List')">
			<#else>
				onclick="${x['buttonCode']}();">
			</#if>
				<i class="icon-${x['buttonIcon']?if_exists?default('pictures')}"></i>${x['buttonName']}	
			</div>
		</#if>
	</#list>
	</span>
...
<#if  (config_queryList?size >0)>
	<#if config_querymode == "group" >
		<span style="float:right">
			<div class="btn btn-default" style="margin-right:5px;" onclick="${config_id}Listsearch()">
				<i class="icon-search"></i>查询
			</div>
			<div class="btn btn-default" style="margin-right:5px;" onclick="${config_id}searchReset('${config_id}List')">
				<i class="icon-share-alt"></i>重置
			</div>
		</span>
	</#if>
